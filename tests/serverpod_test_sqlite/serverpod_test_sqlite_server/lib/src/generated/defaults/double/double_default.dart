...
class DoubleDefaultRepository {
  ...
  /// Emits [DoubleDefault]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `DoubleDefault.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<DoubleDefault>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<DoubleDefaultTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<DoubleDefault>(
      where: where?.call(DoubleDefault.t),
      orderBy: orderBy?.call(DoubleDefault.t),
      orderByList: orderByList?.call(DoubleDefault.t),
      limit: limit,
