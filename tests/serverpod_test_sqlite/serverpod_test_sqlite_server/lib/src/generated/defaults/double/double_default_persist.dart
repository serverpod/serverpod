...
class DoubleDefaultPersistRepository {
  ...
  /// Emits [DoubleDefaultPersist]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `DoubleDefaultPersist.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<DoubleDefaultPersist>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<DoubleDefaultPersistTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<DoubleDefaultPersist>(
      where: where?.call(DoubleDefaultPersist.t),
      orderBy: orderBy?.call(DoubleDefaultPersist.t),
      orderByList: orderByList?.call(DoubleDefaultPersist.t),
      limit: limit,
