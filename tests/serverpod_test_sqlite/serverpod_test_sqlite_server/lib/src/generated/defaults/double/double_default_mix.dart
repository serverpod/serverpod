...
class DoubleDefaultMixRepository {
  ...
  /// Emits [DoubleDefaultMix]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `DoubleDefaultMix.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<DoubleDefaultMix>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<DoubleDefaultMixTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<DoubleDefaultMix>(
      where: where?.call(DoubleDefaultMix.t),
      orderBy: orderBy?.call(DoubleDefaultMix.t),
      orderByList: orderByList?.call(DoubleDefaultMix.t),
      limit: limit,
