...
abstract class UriDefaultPersist
  ...
  UriDefaultPersist copyWith({
    int? id,
    Uri? uriDefaultPersist = const _is.$UndefinedUri(),
  });
  @override
...
class _UriDefaultPersistImpl extends UriDefaultPersist {
  ...
  UriDefaultPersist copyWith({
    ...
    return UriDefaultPersist(
      ...
      uriDefaultPersist: uriDefaultPersist is _is.UndefinedSentinel
          ? this.uriDefaultPersist
          : uriDefaultPersist,
    );
  }
...
class UriDefaultPersistRepository {
  ...
  /// Emits [UriDefaultPersist]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `UriDefaultPersist.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<UriDefaultPersist>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<UriDefaultPersistTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<UriDefaultPersist>(
      where: where?.call(UriDefaultPersist.t),
      orderBy: orderBy?.call(UriDefaultPersist.t),
      orderByList: orderByList?.call(UriDefaultPersist.t),
      limit: limit,
