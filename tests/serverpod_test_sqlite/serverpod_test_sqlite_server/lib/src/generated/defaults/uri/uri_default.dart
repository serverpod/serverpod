...
abstract class UriDefault
  ...
  UriDefault copyWith({
    int? id,
    Uri? uriDefault,
    Uri? uriDefaultNull = const _is.$UndefinedUri(),
  });
  @override
...
class _UriDefaultImpl extends UriDefault {
  ...
  UriDefault copyWith({
    ...
    return UriDefault(
      ...
      uriDefaultNull: uriDefaultNull is _is.UndefinedSentinel
          ? this.uriDefaultNull
          : uriDefaultNull,
    );
  }
...
class UriDefaultRepository {
  ...
  /// Emits [UriDefault]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `UriDefault.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<UriDefault>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<UriDefaultTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<UriDefault>(
      where: where?.call(UriDefault.t),
      orderBy: orderBy?.call(UriDefault.t),
      orderByList: orderByList?.call(UriDefault.t),
      limit: limit,
