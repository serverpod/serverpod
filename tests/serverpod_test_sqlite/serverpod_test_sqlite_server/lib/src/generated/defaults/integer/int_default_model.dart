...
class IntDefaultModelRepository {
  ...
  /// Emits [IntDefaultModel]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `IntDefaultModel.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<IntDefaultModel>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<IntDefaultModelTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<IntDefaultModel>(
      where: where?.call(IntDefaultModel.t),
      orderBy: orderBy?.call(IntDefaultModel.t),
      orderByList: orderByList?.call(IntDefaultModel.t),
      limit: limit,
