...
class IntDefaultPersistRepository {
  ...
  /// Emits [IntDefaultPersist]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `IntDefaultPersist.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<IntDefaultPersist>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<IntDefaultPersistTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<IntDefaultPersist>(
      where: where?.call(IntDefaultPersist.t),
      orderBy: orderBy?.call(IntDefaultPersist.t),
      orderByList: orderByList?.call(IntDefaultPersist.t),
      limit: limit,
