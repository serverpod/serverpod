...
class DurationDefaultRepository {
  ...
  /// Emits [DurationDefault]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `DurationDefault.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<DurationDefault>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<DurationDefaultTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<DurationDefault>(
      where: where?.call(DurationDefault.t),
      orderBy: orderBy?.call(DurationDefault.t),
      orderByList: orderByList?.call(DurationDefault.t),
      limit: limit,
