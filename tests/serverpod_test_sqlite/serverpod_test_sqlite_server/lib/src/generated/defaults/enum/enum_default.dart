...
class EnumDefaultRepository {
  ...
  /// Emits [EnumDefault]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `EnumDefault.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<EnumDefault>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<EnumDefaultTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<EnumDefault>(
      where: where?.call(EnumDefault.t),
      orderBy: orderBy?.call(EnumDefault.t),
      orderByList: orderByList?.call(EnumDefault.t),
      limit: limit,
