...
class UuidDefaultMixRepository {
  ...
  /// Emits [UuidDefaultMix]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `UuidDefaultMix.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<UuidDefaultMix>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<UuidDefaultMixTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<UuidDefaultMix>(
      where: where?.call(UuidDefaultMix.t),
      orderBy: orderBy?.call(UuidDefaultMix.t),
      orderByList: orderByList?.call(UuidDefaultMix.t),
      limit: limit,
