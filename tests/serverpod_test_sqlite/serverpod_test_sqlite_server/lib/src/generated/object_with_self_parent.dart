...
class ObjectWithSelfParentRepository {
  ...
  /// Emits [ObjectWithSelfParent]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ObjectWithSelfParent.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ObjectWithSelfParent>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ObjectWithSelfParentTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ObjectWithSelfParent>(
      where: where?.call(ObjectWithSelfParent.t),
      orderBy: orderBy?.call(ObjectWithSelfParent.t),
      orderByList: orderByList?.call(ObjectWithSelfParent.t),
      limit: limit,
