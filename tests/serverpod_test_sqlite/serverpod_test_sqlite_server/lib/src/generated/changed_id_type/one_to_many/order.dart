...
class OrderUuidRepository {
  ...
  /// Emits [OrderUuid]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `OrderUuid.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<OrderUuid>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<OrderUuidTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<OrderUuid>(
      where: where?.call(OrderUuid.t),
      orderBy: orderBy?.call(OrderUuid.t),
      orderByList: orderByList?.call(OrderUuid.t),
      limit: limit,
