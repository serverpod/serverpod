...
abstract class StudentUuid
  ...
  StudentUuid copyWith({
    ...
    String? name,
    List<_ih6xbg05.EnrollmentInt>? enrollments =
        const _is.$UndefinedList<_ih6xbg05.EnrollmentInt>(),
  });
  @override
...
class StudentUuidRepository {
  ...
  /// Emits [StudentUuid]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `StudentUuid.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<StudentUuid>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<StudentUuidTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<StudentUuid>(
      where: where?.call(StudentUuid.t),
      orderBy: orderBy?.call(StudentUuid.t),
      orderByList: orderByList?.call(StudentUuid.t),
      limit: limit,
