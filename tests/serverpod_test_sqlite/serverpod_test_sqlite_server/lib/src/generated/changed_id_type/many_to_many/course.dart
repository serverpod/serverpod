...
class CourseUuidRepository {
  ...
  /// Emits [CourseUuid]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `CourseUuid.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<CourseUuid>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<CourseUuidTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<CourseUuid>(
      where: where?.call(CourseUuid.t),
      orderBy: orderBy?.call(CourseUuid.t),
      orderByList: orderByList?.call(CourseUuid.t),
      limit: limit,
