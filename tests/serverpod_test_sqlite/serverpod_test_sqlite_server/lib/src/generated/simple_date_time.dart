...
class SimpleDateTimeRepository {
  ...
  /// Emits [SimpleDateTime]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `SimpleDateTime.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<SimpleDateTime>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<SimpleDateTimeTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<SimpleDateTime>(
      where: where?.call(SimpleDateTime.t),
      orderBy: orderBy?.call(SimpleDateTime.t),
      orderByList: orderByList?.call(SimpleDateTime.t),
      limit: limit,
