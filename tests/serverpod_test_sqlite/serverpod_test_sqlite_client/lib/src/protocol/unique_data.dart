...
class UniqueDataRepository {
  ...
  /// Emits [UniqueData]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `UniqueData.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<UniqueData>> watch(
    _isd.DatabaseSession session, {
    _isd.WhereExpressionBuilder<UniqueDataTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<UniqueData>(
      where: where?.call(UniqueData.t),
      orderBy: orderBy?.call(UniqueData.t),
      orderByList: orderByList?.call(UniqueData.t),
      limit: limit,
