...
class ChapterRepository {
  ...
  /// Emits [Chapter]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `Chapter.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<Chapter>> watch(
    _isd.DatabaseSession session, {
    _isd.WhereExpressionBuilder<ChapterTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<Chapter>(
      where: where?.call(Chapter.t),
      orderBy: orderBy?.call(Chapter.t),
      orderByList: orderByList?.call(Chapter.t),
      limit: limit,
