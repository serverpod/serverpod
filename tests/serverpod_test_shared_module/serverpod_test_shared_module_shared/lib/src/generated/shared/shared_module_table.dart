...
class SharedModuleTableRepository {
  ...
  /// Emits [SharedModuleTable]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `SharedModuleTable.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<SharedModuleTable>> watch(
    _isd.DatabaseSession session, {
    _isd.WhereExpressionBuilder<SharedModuleTableTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<SharedModuleTable>(
      where: where?.call(SharedModuleTable.t),
      orderBy: orderBy?.call(SharedModuleTable.t),
      orderByList: orderByList?.call(SharedModuleTable.t),
      limit: limit,
