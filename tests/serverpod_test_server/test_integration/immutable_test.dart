...
void main() {
  withServerpod(
    ...
    test('then inserted object is equal to database fetched object', () async {
      var insertedObject = await insertFuture;
  ...
  withServerpod('Given an immutable model stored in the database',
    ...
    setUp(() async {
      ...
      insertedObject = await ImmutableObjectWithTable.db.insertRow(
          session, ImmutableObjectWithTable(variable: 'initial value'));
    });

    test('then two identical fetches return equal objects', () async {
      var fetchedObject1 = await ImmutableObjectWithTable.db
          .findById(session, insertedObject.id!);
      var fetchedObject2 = await ImmutableObjectWithTable.db
          .findById(session, insertedObject.id!);
