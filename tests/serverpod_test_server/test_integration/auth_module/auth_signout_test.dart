...
void main() {
  ...
  withServerpod('Given a user signed in to multiple devices', (
    sessionBuilder,
    endpoints,
  ) {
    ...
    tearDown(() async {
      await AuthKey.db.deleteWhere(session, where: (_) => Constant.bool(true));
    });

    test('when signing out then user is signed out of all devices', () async {
  ...
  withServerpod('Given two users signed in to multiple devices', (
    sessionBuilder,
    endpoints,
  ) {
    late Session session;
