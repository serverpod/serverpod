...
void main() {
  withServerpod('Given calling `copyWith` on the session builder', (
    sessionBuilder,
    endpoints,
  ) {
    group('when setting a new shared session builder on the group level', () {
      TestSessionBuilder modifiedSessionBuilder = sessionBuilder.copyWith(
        authentication: AuthenticationOverride.authenticationInfo('123', {}),
      );
      ...
      test(
        ...
        () async {
          await expectLater(
            ...
            completion(
              isA<AuthenticationInfo>().having((a) => a.userId, 'userId', 123),
            ),
          );
        },
      );
