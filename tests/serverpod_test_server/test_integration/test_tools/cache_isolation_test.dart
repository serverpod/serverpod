...
void main() {
  withServerpod('Given withServerpod with cache operations', (
    sessionBuilder,
    endpoints,
  ) {
    ...
    group('when tests store data in localPrio cache', () {
      ...
      test('then localPrio cache should be empty from previous test', () async {
        // This test should find cache empty due to automatic cleanup
    ...
    group('when cache data is set in setUp', () {
      ...
      test('then setup cache data should be available in first test', () async {
        final cached = await endpoints.testTools.getFromLocalCache(
          sessionBuilder,
          'setup-key',
        );
