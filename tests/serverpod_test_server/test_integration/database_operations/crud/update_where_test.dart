...
void main() {
  withServerpod('Given database entries with basic matching criteria', (
    sessionBuilder,
    endpoints,
  ) {
    ...
    group(
      ...
      () {
        ...
        test('then non-selected columns remain unchanged', () {
          ...
          expect(emails, containsAll(['first', 'duplicate']));
        });
      },
    );
    ...
    group(
      ...
      () {
        ...
        test('then only matching rows are updated in the database', () async {
          ...
          expect(emails, containsAll(['second', 'third']));
        });
    ...
    group('when updating where criteria matches for multiple columns', () {
      ...
      setUp(() async {
        updated = await Types.db.updateWhere(
          session,
          columnValues: (t) => [t.anInt(updatedInt), t.aString(updatedString)],
          where: (t) => t.anInt.equals(1),
        );
      });
      ...
      test('then only selected columns are updated in the database', () async {
  ...
  withServerpod('Given database entries for transaction testing', (
    sessionBuilder,
    endpoints,
  ) {
    var session = sessionBuilder.build();
  ...
  withServerpod('Given no matching database entries', (
    sessionBuilder,
    endpoints,
  ) {
    var session = sessionBuilder.build();
    ...
    setUp(() async {
      await Types.db.insert(session, [Types(anInt: 1, aString: 'test')]);
    });
  ...
  withServerpod('Given database entries with null values', (
    sessionBuilder,
    endpoints,
  ) {
    var session = sessionBuilder.build();
  ...
  withServerpod('Given database entries with non-null values', (
    sessionBuilder,
    endpoints,
  ) {
    var session = sessionBuilder.build();
  ...
  withServerpod('Given database entries for pagination operations', (
    sessionBuilder,
    endpoints,
  ) {
    var session = sessionBuilder.build();
  ...
  withServerpod('Given database entries with all supported data types', (
    sessionBuilder,
    endpoints,
  ) {
    var session = sessionBuilder.build();
    ...
    setUp(() async {
      await Types.db.insert(session, [
        Types(
          ...
          aMap: {1: 10, 2: 20},
          aSet: {1, 2, 3},
          aRecord: ('original', optionalUri: Uri.parse('https://example.com')),
        ),
        Types(
          anInt: 2,
          aBool: false,
          ...
          aUuid: UuidValue.fromString('456e7890-e12b-34c5-a678-901234567890'),
        ...
        updated = await Types.db.updateWhere(
          ...
          columnValues:
              (t) => [
                ...
                t.aString('updated'),
                t.aByteData(
                  ByteData.view(Uint8List.fromList([4, 5, 6]).buffer),
                ),
