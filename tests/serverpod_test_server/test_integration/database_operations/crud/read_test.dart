...
void main() async {
  ...
  group('Given an empty database', () {
    test(
      ...
      () async {
        var retrieved = await SimpleData.db.findById(session, 1);
  ...
  test(
    ...
    () async {
      ...
      var retrieved = await SimpleData.db.findById(session, inserted.id!);
  ...
  test(
    ...
    () async {
      ...
      var inserted = await Types.db.insertRow(session, object);

      var retrieved = await Types.db.findById(session, inserted.id!);
  ...
  test(
    ...
    () async {
      var object = Types(aDateTime: DateTime.utc(2024, 12, 24, 23, 30));
      var inserted = await Types.db.insertRow(session, object);
  ...
  test(
    ...
    () async {
      var object = Types(aDuration: Duration(hours: 1, minutes: 2, seconds: 3));
      var inserted = await Types.db.insertRow(session, object);
  ...
  test(
    ...
    () async {
      ...
      var object = Types(aUri: uri);
      var inserted = await Types.db.insertRow(session, object);

      var retrieved = await Types.db.findById(session, inserted.id!);

      expect(retrieved?.aUri, uri);
    },
  );
  ...
  test(
    ...
    () async {
      var object = Types(aBigInt: BigInt.two);
      var inserted = await Types.db.insertRow(session, object);

      var retrieved = await Types.db.findById(session, inserted.id!);

      expect(retrieved?.aBigInt, BigInt.two);
    },
  );
  ...
  test(
    ...
    () async {
      ...
      expect(retrieved?.aRecord, (
        'hello',
        optionalUri: Uri.parse('https://serverpod.dev'),
      ));
    },
  );
  ...
  test(
    ...
    () async {
      var object = Types(anEnum: TestEnum.two);
      var inserted = await Types.db.insertRow(session, object);

      var retrieved = await Types.db.findById(session, inserted.id!);

      expect(retrieved?.anEnum, TestEnum.two);
    },
  );
  ...
  test(
    ...
    () async {
      var object = Types(aStringifiedEnum: TestEnumStringified.three);
      var inserted = await Types.db.insertRow(session, object);
