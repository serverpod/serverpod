...
void main() {
  withServerpod('Given a database entry with all basic fields', (
    sessionBuilder,
    endpoints,
  ) {
    ...
    setUp(() async {
      existingEntry = await Types.db.insertRow(
        session,
        Types(aBool: originalBool, anInt: originalInt, aString: originalString),
      );
    });
    ...
    group('when updating by id with no columns specified', () {
      ...
      test('then the row remains unchanged in the database', () async {
    ...
    group('when updating by id for a single column', () {
      ...
      test('then only that column is updated', () async {
        var dbRow = await Types.db.findById(session, existingEntry.id!);
        expect(dbRow, isNotNull);
        expect(dbRow!.anInt, updatedInt);
      });

      test('then other columns are not updated', () async {
        var dbRow = await Types.db.findById(session, existingEntry.id!);
        expect(dbRow, isNotNull);
        expect(dbRow!.aBool, originalBool);
        expect(dbRow.aString, originalString);
    ...
    group('when updating by id for multiple columns', () {
      ...
      test('then only the selected columns are updated', () async {
        var dbRow = await Types.db.findById(session, existingEntry.id!);
        expect(dbRow, isNotNull);
        expect(dbRow!.anInt, updatedInt);
  ...
  withServerpod('Given a database entry with non-null values', (
    sessionBuilder,
    endpoints,
  ) {
    var session = sessionBuilder.build();
  ...
  withServerpod('Given a database entry with null values', (
    sessionBuilder,
    endpoints,
  ) {
    var session = sessionBuilder.build();
  ...
  withServerpod('Given a database entry with all supported data types', (
    sessionBuilder,
    endpoints,
  ) {
    var session = sessionBuilder.build();
  ...
  withServerpod('Given a non-existent database entry', (
    testSession,
    endpoints,
  ) {
