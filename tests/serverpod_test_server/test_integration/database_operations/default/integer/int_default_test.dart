...
void main() async {
  ...
  group('Given a class with "default" fields,', () {
    tearDownAll(
      () async =>
          IntDefault.db.deleteWhere(session, where: (_) => Constant.bool(true)),
    );
    ...
    test(
      ...
      () async {
        ...
        var databaseObject = await IntDefault.db.insertRow(session, object);
        expect(databaseObject.intDefault, equals(10));
      },
    );
    ...
    test(
      ...
      () async {
        var object = IntDefault();
        var databaseObject = await IntDefault.db.insertRow(session, object);
        expect(databaseObject.intDefaultNull, equals(20));
      },
    );
    ...
    test(
      ...
      () async {
        var specificObject = IntDefault(intDefault: 30);
        var specificDatabaseObject = await IntDefault.db.insertRow(
          session,
          specificObject,
        );
        expect(specificDatabaseObject.intDefault, equals(30));
      },
    );
    ...
    test(
      ...
      () async {
        var specificObject = IntDefault(intDefaultNull: 40);
        var specificDatabaseObject = await IntDefault.db.insertRow(
          session,
          specificObject,
        );
        expect(specificDatabaseObject.intDefaultNull, equals(40));
      },
    );
  });
}
