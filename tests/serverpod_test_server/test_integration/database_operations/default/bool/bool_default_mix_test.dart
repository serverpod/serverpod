...
void main() async {
  ...
  group('Given a class with mixed default fields,', () {
    tearDownAll(() async => BoolDefaultMix.db.deleteWhere(
          session,
          where: (_) => Constant.bool(true),
        ));

    test(
      'when creating a record in the database with an unsafe query, then the "boolDefaultAndDefaultModel" field value should match the provided value',
      () async {
    ...
    test(
      ...
      () async {
        ...
        expect(
            specificDatabaseObject.boolDefaultModelAndDefaultPersist, isFalse);
      },
    );
  });
}
