...
void main() async {
  ...
  group(
    ...
    test(
        'when counting entities filtered on none of nested many relation then result is as expected.',
        () async {
    ...
    test(
        'when fetching entities filtered on filtered none of nested many relation then result is as expected.',
        () async {
      var players = await Player.db.insert(session, [
        Player(name: 'Alex'),
        Player(name: 'Viktor'),
