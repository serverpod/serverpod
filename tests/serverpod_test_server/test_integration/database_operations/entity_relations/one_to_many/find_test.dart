...
Future<void> _createTestDatabase(Session session) async {
  ...
  var orders = [
    ...
    Order(description: 'Prem: Order 7', customerId: customers[3].id!),
    Order(description: 'Basic: Order 8', customerId: customers[3].id!),
    Order(description: 'Basic: Order 9', customerId: customers[3].id!),
  ];
...
void main() async {
  ...
  group('Given find query ', () {
    test('when filtering on many relation count then result is as expected.',
      ...
      expect(customerNames, containsAll(['Alex', 'Isak', 'Lisa']));
    });

    test(
        'when filtering on many relation count in combination with other filter then result is as expected.',
        () async {
      var customers = await Customer.db.find(
        session,
        // All customers with more than two orders or name 'Viktor'
        where: (c) => (c.orders.count() > 2) | c.name.equals('Viktor'),
      );

      var customerNames = customers.map((e) => e.name);
      expect(customerNames, hasLength(3));
      expect(customerNames, containsAll(['Alex', 'Lisa', 'Viktor']));
    });
    ...
    test('when filtering on nested count then result is as expected', () async {
      ...
      expect(customerNames, contains('Alex'));
    });
    ...
    test(
      ...
      expect(customerNames, containsAll(['Isak', 'Lisa']));
    });
    ...
    test(
      ...
      var customers = await Customer.db.find(
        session,
        // All customers with more than two orders but less than four
        where: (c) => (c.orders.count() > 2) & (c.orders.count() < 4),
      );
    ...
    test(
        'when filtering on multiple filtered many relation count then result is as expected.',
        () async {
      var customers = await Customer.db.find(
        session,
      ...
      var customerNames = customers.map((e) => e.name);
      expect(customerNames, hasLength(1));
      expect(customerNames, contains('Lisa'));
    });
    ...
    test(
        'when filtering on nested filtered count and ordering by count then result is as expected',
        () async {
      var customers = await Customer.db.find(
        session,
        // All customers with more than one order with more than one comments starting with 'del'
        where: (c) => c.orders.count((o) => o.description.ilike('prem%')) > 1,
        // Order by number of orders with description starting with 'basic'
      ...
      var customerNames = customers.map((e) => e.name);
      expect(customerNames, hasLength(2));
      expect(customerNames.first, 'Lisa');
