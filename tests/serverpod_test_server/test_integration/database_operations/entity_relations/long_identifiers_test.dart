...
void main() async {
  ...
  group('Given a model with a list relation to model with max length named fields', () {
    ...
    test(
      ...
      () async {
        var multipleMaxFieldNames = await MultipleMaxFieldName.db
            .insert(session, [
        ...
        var relationToMultipleMaxFieldNames =
            await RelationToMultipleMaxFieldName.db.insert(session, [
              RelationToMultipleMaxFieldName(name: 'First'),
              RelationToMultipleMaxFieldName(name: 'Second'),
              RelationToMultipleMaxFieldName(name: 'Third'),
            ]);
    ...
    test(
      ...
      () async {
        var longImplicitField = await LongImplicitIdField.db.insertRow(
          session,
          LongImplicitIdField(name: 'HasField'),
        );
  ...
  group('Given a self relation', () {
    ...
    test(
      ...
      () async {
        var posts = await Post.db.insert(session, [
          Post(content: '0'),
          Post(content: '1'),
          Post(content: '2'),
          Post(content: '3'),
              ...
              next: Post.include(
                next: Post.include(
                  next: Post.include(
                    next: Post.include(
                      next: Post.include(
                        next: Post.include(next: Post.include()),
                      ),
                    ),
                  ),
                ),
