    ...
    test(
      ...
      () async {
        await session.db.transaction((transaction) async {
          ...
          var data = await UniqueData.db.find(session);
    ...
    test(
      ...
      () async {
        await session.db.transaction((transaction) async {
          ...
          var data = await UniqueData.db.findFirstRow(session);
    ...
    test(
      ...
      () async {
        await session.db.transaction((transaction) async {
          ...
          var data = await UniqueData.db.findById(session, insertedData.id!);
  ...
  group('Given deeply nested list relation ', () {
    ...
    test('when creating all objects inside a transaction '
      ...
      await session.db.transaction((transaction) async {
        ...
        await Organization.db.attach.people(session, flutter, [
          person3,
          person4,
        ], transaction: transaction);
