    ...
    test(
      ...
      () async {
        ...
        var companiesFetched = await CompanyUuid.db.find(
          ...
          orderByList:
              (t) => [db.Order(column: t.town.name), db.Order(column: t.name)],
        );

        var companyNames = companiesFetched.map((c) => c.name);
        expect(companyNames, ['Apple', 'Google', 'Serverpod']);
      },
    );
  });
  ...
  group('Given models with nested one to one relations', () {
    ...
    test(
      ...
      () async {
        ...
        expect(citizenNames, ['Lina', 'Marc', 'Alex', 'Isak', 'Yuko']);
      },
    );
  });
