    ...
    () {
      ...
      setUpAll(() async {
        ...
        citizensIncludingAddress = await CitizenInt.db.find(
          session,
          orderBy: (t) => t.id,
          include: CitizenInt.include(address: AddressUuid.include()),
        );
      });
    ...
    () {
      ...
      setUpAll(() async {
        ...
        addresses = await AddressUuid.db.find(
          session,
          orderBy: (t) => t.id,
          include: AddressUuid.include(inhabitant: CitizenInt.include()),
        );
      });
  ...
  group('Given models with a named self relation', () {
    ...
    setUpAll(() async {
      ...
      posts = await Post.db.find(
        session,
        include: Post.include(previous: Post.include(), next: Post.include()),
      );
    });
