...
import 'package:serverpod_open_metrics_server/serverpod_open_metrics_server.dart';
...
void main() {
  group('metricsMiddleware - basic metrics collection', () {
    ...
    test('serves /metrics endpoint with correct content type', () async {
    ...
    test('collects http_requests_total metric', () async {
      // Make a request
    ...
    test('collects http_request_duration_seconds metric', () async {
      // Make a request
      await httpClient.post(
        endpoint,
        headers: {'Content-Type': 'application/json'},
    ...
    test('collects http_requests_in_flight metric', () async {
      final metricsResponse = await httpClient.get(metricsEndpoint);
      final metrics = metricsResponse.body;
    ...
    test('collects process_start_time_seconds metric', () async {
      final metricsResponse = await httpClient.get(metricsEndpoint);
      final metrics = metricsResponse.body;
    ...
    test('normalizes paths to prevent unbounded cardinality', () async {
    ...
    test('records sub-millisecond timing precision', () async {
      // Make a fast request
      await httpClient.post(
        endpoint,
        headers: {'Content-Type': 'application/json'},
    ...
    test('tracks different HTTP methods separately', () async {
  ...
  group('metricsMiddleware - custom configuration', () {
    test('accepts custom registry with custom metrics', () async {
      final customRegistry = MetricRegistry();

      // Add custom metric
      final myCounter = customRegistry.counter(
        'my_custom_total',
    ...
    test('accepts custom metrics path', () async {
      final portZeroConfig = ServerConfig(
        port: 0,
        publicScheme: 'http',
        publicHost: 'localhost',
    ...
    test('accepts custom histogram buckets', () async {
    ...
    test('can disable in-flight tracking', () async {
      final portZeroConfig = ServerConfig(
        port: 0,
        publicScheme: 'http',
        publicHost: 'localhost',
  ...
  group('metricsMiddleware - parameter validation', () {
    test('throws ArgumentError for maxPathPatterns = 0', () {
      expect(
    ...
    test('throws ArgumentError for maxLabelCardinality = 0', () {
      expect(
    ...
    test('accepts maxPathPatterns = 1 (minimum valid value)', () {
      final middleware = metricsMiddleware(maxPathPatterns: 1);
      expect(middleware, isA<Middleware>());
    });

    test('accepts maxLabelCardinality = 1 (minimum valid value)', () {
      final middleware = metricsMiddleware(maxLabelCardinality: 1);
      expect(middleware, isA<Middleware>());
    });

    test('accepts null maxLabelCardinality (uses default)', () {
      final middleware = metricsMiddleware(maxLabelCardinality: null);
      expect(middleware, isA<Middleware>());
    });

    test('fails fast during configuration, not during first request', () {
  ...
  group('metricsMiddleware - registry conflict handling', () {
    test(
      'throws descriptive error when registry has conflicting metric type',
      () {
    ...
    test('reuses existing compatible metrics from registry', () {
      final registry = MetricRegistry();

      // Pre-register the metrics with compatible types
      final existingCounter = registry.labeledCounter(
        'http_requests_total',
        help: 'Pre-existing counter',
        labelNames: ['method', 'path', 'status'],
      );

      final existingHistogram = registry.labeledHistogram(
        'http_request_duration_seconds',
    ...
    test('throws original error for invalid histogram buckets', () {
    ...
    test('throws original error for empty histogram buckets', () {
      expect(
  ...
  group('metricsMiddleware - cardinality protection', () {
    test('calculates default maxLabelCardinality based on maxPathPatterns', () {
    ...
    test('accepts custom maxLabelCardinality', () {
    ...
    test('enforces maximum path patterns limit', () async {
      final portZeroConfig = ServerConfig(
        port: 0,
        publicScheme: 'http',
        publicHost: 'localhost',
  ...
  group('metricsMiddleware - error handling', () {
    ...
    test('tracks metrics even when errors occur', () async {
    ...
    test('decrements in-flight counter on error', () async {
      // Make request that will trigger error
      try {
        await httpClient.post(
          endpoint,
  ...
  group('metricsMiddleware - multiple middleware interaction', () {
    ...
    test('metrics middleware works correctly in middleware chain', () async {
      await httpClient.post(
        endpoint,
        headers: {'Content-Type': 'application/json'},
        body: '{"value": 42}',
  ...
  group('metricsMiddleware - metrics endpoint behavior', () {
    test('metrics endpoint does not get tracked in metrics', () async {
      final portZeroConfig = ServerConfig(
        port: 0,
        publicScheme: 'http',
        publicHost: 'localhost',
  ...
  group('metricsMiddleware - no server required', () {
    ...
    test('factory function accepts custom registry', () {
      final registry = MetricRegistry();
      final middleware = metricsMiddleware(registry: registry);
      expect(middleware, isA<Middleware>());
    });

    test('factory function accepts all configuration options', () {
      final registry = MetricRegistry();
  ...
  group('metricsMiddleware - IP access control', () {
    ...
    test('works with custom registry and IP restrictions', () async {
      ...
      final customRegistry = MetricRegistry();
      final customCounter = customRegistry.counter(
        'my_custom_metric',
        help: 'Custom metric',
      );
