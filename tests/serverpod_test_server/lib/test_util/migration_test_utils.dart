...
abstract class MigrationTestUtils {
  static Future<void> createInitialState({
    ...
    for (var protocols in migrationProtocols) {
      ...
      assert(exitCode == 0, 'Failed to create migration.');
    }

    assert(await runApplyMigrations() == 0, 'Failed to create migration.');
  }
  ...
  static Future<int> createMigrationFromProtocols({
    ...
    protocols.forEach((fileName, contents) {
      var protocolFile = File(
        path.join(_migrationProtocolTestDirectory().path, '$fileName.yaml'),
      );
  ...
  static String readMigrationRegistryFile() {
    var migrationRegistryFile = File(
      path.join(_migrationsProjectDirectory().path, 'migration_registry.txt'),
    );
  ...
  static MigrationRegistry loadMigrationRegistry() {
    return MigrationRegistry.load(_migrationsProjectDirectory());
  }
  ...
  static Directory _repairMigrationDirectory() =>
      Directory(path.join(Directory.current.path, 'repair-migration'));

  static Directory _migrationsProjectDirectory() =>
