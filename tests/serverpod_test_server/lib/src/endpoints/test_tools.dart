...
class TestToolsEndpoint extends Endpoint {
  ...
  Stream<int> returnsStream(Session session, int n) {
    return Stream<int>.fromIterable(List<int>.generate(n, (index) => index));
  }
  ...
  Future<void> createSimpleData(Session session, int data) async {
    await SimpleData.db.insertRow(session, SimpleData(num: data));
  }
  ...
  Future<Types> echoTypes(Session session, Types typesModel) async {
    return typesModel;
  }
