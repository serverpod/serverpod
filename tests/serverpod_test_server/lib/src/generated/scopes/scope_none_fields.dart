...
class ScopeNoneFieldsRepository {
  ...
  /// Emits [ScopeNoneFields]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ScopeNoneFields.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ScopeNoneFields>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ScopeNoneFieldsTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ScopeNoneFields>(
      where: where?.call(ScopeNoneFields.t),
      orderBy: orderBy?.call(ScopeNoneFields.t),
      orderByList: orderByList?.call(ScopeNoneFields.t),
      limit: limit,
