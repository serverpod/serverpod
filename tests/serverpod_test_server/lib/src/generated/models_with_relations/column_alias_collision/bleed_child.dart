...
class BleedChildRepository {
  ...
  /// Emits [BleedChild]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `BleedChild.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<BleedChild>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<BleedChildTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<BleedChild>(
      where: where?.call(BleedChild.t),
      orderBy: orderBy?.call(BleedChild.t),
      orderByList: orderByList?.call(BleedChild.t),
      limit: limit,
