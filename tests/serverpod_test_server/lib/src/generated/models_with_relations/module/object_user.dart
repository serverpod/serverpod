...
class ObjectUserRepository {
  ...
  /// Emits [ObjectUser]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ObjectUser.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ObjectUser>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ObjectUserTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ObjectUser>(
      where: where?.call(ObjectUser.t),
      orderBy: orderBy?.call(ObjectUser.t),
      orderByList: orderByList?.call(ObjectUser.t),
      limit: limit,
