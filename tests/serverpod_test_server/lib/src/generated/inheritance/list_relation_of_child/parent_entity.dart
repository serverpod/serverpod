...
abstract class ParentEntity
  ...
  ParentEntity copyWith({
    int? id,
    List<_i41rqetj.ChildEntity>? children =
        const _is.$UndefinedList<_i41rqetj.ChildEntity>(),
  });
  @override
...
class ParentEntityRepository {
  ...
  /// Emits [ParentEntity]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ParentEntity.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ParentEntity>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ParentEntityTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ParentEntity>(
      where: where?.call(ParentEntity.t),
      orderBy: orderBy?.call(ParentEntity.t),
      orderByList: orderByList?.call(ParentEntity.t),
      limit: limit,
