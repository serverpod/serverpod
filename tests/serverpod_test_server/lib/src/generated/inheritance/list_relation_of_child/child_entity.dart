...
class ChildEntityRepository {
  ...
  /// Emits [ChildEntity]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ChildEntity.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ChildEntity>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ChildEntityTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ChildEntity>(
      where: where?.call(ChildEntity.t),
      orderBy: orderBy?.call(ChildEntity.t),
      orderByList: orderByList?.call(ChildEntity.t),
      limit: limit,
