...
class ParentClassRepository {
  ...
  /// Emits [ParentClass]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ParentClass.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ParentClass>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ParentClassTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ParentClass>(
      where: where?.call(ParentClass.t),
      orderBy: orderBy?.call(ParentClass.t),
      orderByList: orderByList?.call(ParentClass.t),
      limit: limit,
