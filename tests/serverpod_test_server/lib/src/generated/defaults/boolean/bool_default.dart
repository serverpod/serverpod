...
class BoolDefaultRepository {
  ...
  /// Emits [BoolDefault]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `BoolDefault.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<BoolDefault>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<BoolDefaultTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<BoolDefault>(
      where: where?.call(BoolDefault.t),
      orderBy: orderBy?.call(BoolDefault.t),
      orderByList: orderByList?.call(BoolDefault.t),
      limit: limit,
