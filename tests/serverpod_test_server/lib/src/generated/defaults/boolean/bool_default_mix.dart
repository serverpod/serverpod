...
class BoolDefaultMixRepository {
  ...
  /// Emits [BoolDefaultMix]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `BoolDefaultMix.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<BoolDefaultMix>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<BoolDefaultMixTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<BoolDefaultMix>(
      where: where?.call(BoolDefaultMix.t),
      orderBy: orderBy?.call(BoolDefaultMix.t),
      orderByList: orderByList?.call(BoolDefaultMix.t),
      limit: limit,
