...
class BoolDefaultModelRepository {
  ...
  /// Emits [BoolDefaultModel]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `BoolDefaultModel.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<BoolDefaultModel>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<BoolDefaultModelTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<BoolDefaultModel>(
      where: where?.call(BoolDefaultModel.t),
      orderBy: orderBy?.call(BoolDefaultModel.t),
      orderByList: orderByList?.call(BoolDefaultModel.t),
      limit: limit,
