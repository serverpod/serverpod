...
class UriDefaultMixRepository {
  ...
  /// Emits [UriDefaultMix]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `UriDefaultMix.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<UriDefaultMix>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<UriDefaultMixTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<UriDefaultMix>(
      where: where?.call(UriDefaultMix.t),
      orderBy: orderBy?.call(UriDefaultMix.t),
      orderByList: orderByList?.call(UriDefaultMix.t),
      limit: limit,
