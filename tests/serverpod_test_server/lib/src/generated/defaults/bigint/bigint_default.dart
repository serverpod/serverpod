...
class BigIntDefaultRepository {
  ...
  /// Emits [BigIntDefault]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `BigIntDefault.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<BigIntDefault>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<BigIntDefaultTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<BigIntDefault>(
      where: where?.call(BigIntDefault.t),
      orderBy: orderBy?.call(BigIntDefault.t),
      orderByList: orderByList?.call(BigIntDefault.t),
      limit: limit,
