...
class EnumDefaultPersistRepository {
  ...
  /// Emits [EnumDefaultPersist]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `EnumDefaultPersist.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<EnumDefaultPersist>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<EnumDefaultPersistTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<EnumDefaultPersist>(
      where: where?.call(EnumDefaultPersist.t),
      orderBy: orderBy?.call(EnumDefaultPersist.t),
      orderByList: orderByList?.call(EnumDefaultPersist.t),
      limit: limit,
