...
class DateTimeDefaultRepository {
  ...
  /// Emits [DateTimeDefault]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `DateTimeDefault.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<DateTimeDefault>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<DateTimeDefaultTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<DateTimeDefault>(
      where: where?.call(DateTimeDefault.t),
      orderBy: orderBy?.call(DateTimeDefault.t),
      orderByList: orderByList?.call(DateTimeDefault.t),
      limit: limit,
