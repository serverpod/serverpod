...
class DurationDefaultMixRepository {
  ...
  /// Emits [DurationDefaultMix]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `DurationDefaultMix.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<DurationDefaultMix>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<DurationDefaultMixTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<DurationDefaultMix>(
      where: where?.call(DurationDefaultMix.t),
      orderBy: orderBy?.call(DurationDefaultMix.t),
      orderByList: orderByList?.call(DurationDefaultMix.t),
      limit: limit,
