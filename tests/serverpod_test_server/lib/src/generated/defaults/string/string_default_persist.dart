...
class StringDefaultPersistRepository {
  ...
  /// Emits [StringDefaultPersist]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `StringDefaultPersist.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<StringDefaultPersist>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<StringDefaultPersistTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<StringDefaultPersist>(
      where: where?.call(StringDefaultPersist.t),
      orderBy: orderBy?.call(StringDefaultPersist.t),
      orderByList: orderByList?.call(StringDefaultPersist.t),
      limit: limit,
