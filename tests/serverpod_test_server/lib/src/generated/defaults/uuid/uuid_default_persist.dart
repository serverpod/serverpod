...
abstract class UuidDefaultPersist
  ...
  UuidDefaultPersist copyWith({
    int? id,
    _is.UuidValue? uuidDefaultPersistRandom = const _issu.$UndefinedUuidValue(),
    _is.UuidValue? uuidDefaultPersistRandomV7 =
        const _issu.$UndefinedUuidValue(),
    _is.UuidValue? uuidDefaultPersistStr = const _issu.$UndefinedUuidValue(),
  });
  @override
...
class UuidDefaultPersistRepository {
  ...
  /// Emits [UuidDefaultPersist]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `UuidDefaultPersist.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<UuidDefaultPersist>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<UuidDefaultPersistTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<UuidDefaultPersist>(
      where: where?.call(UuidDefaultPersist.t),
      orderBy: orderBy?.call(UuidDefaultPersist.t),
      orderByList: orderByList?.call(UuidDefaultPersist.t),
      limit: limit,
