...
class ObjectWithIndexRepository {
  ...
  /// Emits [ObjectWithIndex]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ObjectWithIndex.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ObjectWithIndex>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ObjectWithIndexTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ObjectWithIndex>(
      where: where?.call(ObjectWithIndex.t),
      orderBy: orderBy?.call(ObjectWithIndex.t),
      orderByList: orderByList?.call(ObjectWithIndex.t),
      limit: limit,
