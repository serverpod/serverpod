...
class EmptyModelWithTableRepository {
  ...
  /// Updates a single [EmptyModelWithTable] by its [id] with the specified [columns].
  /// Returns the updated row or null if no row with the given id exists.
  Future<EmptyModelWithTable?> updateById(
    _i1.Session session,
    int id,
    _i1.ColumnValueListBuilder<EmptyModelWithTableTable> columns, {
    _i1.Transaction? transaction,
  }) async {
    return session.db.updateById<EmptyModelWithTable>(
      id,
      columns(EmptyModelWithTable.t),
      transaction: transaction,
    );
  }

  /// Updates all [EmptyModelWithTable]s matching the [where] expression with the specified [columns].
