...
class ServiceRepository {
  ...
  /// Emits [Service]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `Service.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<Service>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ServiceTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<Service>(
      where: where?.call(Service.t),
      orderBy: orderBy?.call(Service.t),
      orderByList: orderByList?.call(Service.t),
      limit: limit,
