...
class EmployeeRepository {
  ...
  /// Emits [Employee]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `Employee.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<Employee>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<EmployeeTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<Employee>(
      where: where?.call(Employee.t),
      orderBy: orderBy?.call(Employee.t),
      orderByList: orderByList?.call(Employee.t),
      limit: limit,
