...
class UpsertTestModelRepository {
  ...
  /// Emits [UpsertTestModel]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `UpsertTestModel.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<UpsertTestModel>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<UpsertTestModelTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<UpsertTestModel>(
      where: where?.call(UpsertTestModel.t),
      orderBy: orderBy?.call(UpsertTestModel.t),
      orderByList: orderByList?.call(UpsertTestModel.t),
      limit: limit,
