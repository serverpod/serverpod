...
class ObjectWithEnumRepository {
  ...
  /// Emits [ObjectWithEnum]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ObjectWithEnum.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ObjectWithEnum>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ObjectWithEnumTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ObjectWithEnum>(
      where: where?.call(ObjectWithEnum.t),
      orderBy: orderBy?.call(ObjectWithEnum.t),
      orderByList: orderByList?.call(ObjectWithEnum.t),
      limit: limit,
