...
class MaxFieldNameRepository {
  ...
  /// Emits [MaxFieldName]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `MaxFieldName.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<MaxFieldName>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<MaxFieldNameTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<MaxFieldName>(
      where: where?.call(MaxFieldName.t),
      orderBy: orderBy?.call(MaxFieldName.t),
      orderByList: orderByList?.call(MaxFieldName.t),
      limit: limit,
