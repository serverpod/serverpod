...
class MultipleMaxFieldNameRepository {
  ...
  /// Emits [MultipleMaxFieldName]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `MultipleMaxFieldName.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<MultipleMaxFieldName>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<MultipleMaxFieldNameTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<MultipleMaxFieldName>(
      where: where?.call(MultipleMaxFieldName.t),
      orderBy: orderBy?.call(MultipleMaxFieldName.t),
      orderByList: orderByList?.call(MultipleMaxFieldName.t),
      limit: limit,
