...
class MultipleMaxFieldNameRepository {
  ...
  /// Updates a single [MultipleMaxFieldName] by its [id] with the specified [columns].
  /// Returns the updated row or null if no row with the given id exists.
  Future<MultipleMaxFieldName?> updateById(
    _i1.Session session,
    int id,
    _i1.ColumnValueListBuilder<MultipleMaxFieldNameTable> columns, {
    _i1.Transaction? transaction,
  }) async {
    return session.db.updateById<MultipleMaxFieldName>(
      id,
      columns(MultipleMaxFieldName.t),
      transaction: transaction,
    );
  }

  /// Updates all [MultipleMaxFieldName]s matching the [where] expression with the specified [columns].
