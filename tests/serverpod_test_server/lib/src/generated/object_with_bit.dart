...
class ObjectWithBitRepository {
  ...
  /// Emits [ObjectWithBit]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ObjectWithBit.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ObjectWithBit>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ObjectWithBitTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ObjectWithBit>(
      where: where?.call(ObjectWithBit.t),
      orderBy: orderBy?.call(ObjectWithBit.t),
      orderByList: orderByList?.call(ObjectWithBit.t),
      limit: limit,
