...
void main() {
  group('Given an authenticated user', () {
    ...
    tearDownAll(() async {
      ...
      await client.authentication.signOutAllDevices();
      secondClient.close();
      client.close();
    });

    test(
        'when calling the deprecated signOut method then the user is signed out from all sessions on all devices',
        () async {
      ...
      // Sign out from all sessions using the deprecated signOut method
    ...
    test(
        'when calling signOutCurrentDevice then only the current device session is signed out and other devices remain signed in',
        () async {
      // Authenticate the first client and store the auth key
      var response = await client.authentication.authenticate(
        'test@foo.bar',
      ...
      await client.authentication.signOutCurrentDevice();
    ...
    test(
        'when calling signOutAllDevices then all sessions on all devices are signed out',
        () async {
      // Authenticate the first client and store the auth key
      var response = await client.authentication.authenticate(
        'test@foo.bar',
