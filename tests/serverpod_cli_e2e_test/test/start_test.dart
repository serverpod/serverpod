...
''', flush: true);
  ...
  group('Given a project with missing passwords config file', () {
    late String sandboxDir;
    var projectName =
        'test_${const Uuid().v4().replaceAll('-', '_').toLowerCase()}';
    var serverDir = path.join(projectName, '${projectName}_server');
    ...
    setUpAll(() async {
      ...
      createDynamicPortConfig(path.join(sandboxDir, serverDir));
      // Delete the passwords.yaml file
    ...
    group('when running serverpod start', () {
      ...
      test(
        'then it exits with a non-zero exit code and logs errors',
        () async {
          await expectLater(serverProcess?.exitCode, completion(isNonZero));
          await expectLater(
            streamSearch!.keywordFound,
            completion(isTrue),
            reason: 'Server did not exit and log errors before timeout.',
          );
        },
      );
    });
