...
void main() {
  ...
  group('Given two authenticated clients with SessionManagers', () {
    ...
    group(
      ...
      test(
          'and refreshing the session on the second client then the second client is signed out in SessionManager and on the server',
          () async {
        expect(
          await secondaryClient.modules.auth.status.isSignedIn(),
          isFalse,
    ...
    group('when calling signOutCurrentDevice on the first SessionManager', () {
      setUp(() async {
        bool result = await primarySessionManager.signOutCurrentDevice();
        assert(result,
      ...
      test(
          'and refreshing the session on the second client then the second client remains signed in in SessionManager and on the server',
          () async {
        expect(
          await secondaryClient.modules.auth.status.isSignedIn(),
          isTrue,
