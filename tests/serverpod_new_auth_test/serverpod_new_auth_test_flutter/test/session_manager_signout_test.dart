...
void main() {
  group(
    ...
    group('when calling the signOutDevice method on the JWT client', () {
      ...
      test('then the user is signed out in the JWT client and on the server.',
          () async {
      ...
      test(
          ...
          () async {
        expect(await jwtClient.auth.refreshAuthKey(force: true),
    ...
    group('when calling the signOutDevice method on the SAS client', () {
      ...
      test('then the user is signed out in the SAS client and on the server.',
          () async {
      ...
      test('then the user can still refresh tokens in the JWT client.',
          () async {
        expect(await jwtClient.auth.refreshAuthKey(force: true),
    ...
    group('when calling the signOutAllDevices method on the JWT client', () {
      ...
      test('then validateAuthentication signs out the user in the SAS client.',
          () async {
    ...
    group('when calling the signOutAllDevices method on the SAS client', () {
      ...
      test('then the user can no longer refresh tokens in the JWT client.',
          () async {
        expect(await jwtClient.auth.refreshAuthKey(force: true),
            RefreshAuthKeyResult.failedUnauthorized);
      });

      test('then validateAuthentication signs out the user in the JWT client.',
          () async {
