    ...
    () {
      ...
      setUpAll(() async {
        result = await Process.run(
          'dart',
          ['run', 'runner.dart'],
          environment: {'SERVERPOD_DATABASE_PASSWORD': dbPassword},
    ...
    () {
      ...
      setUpAll(() async {
        result = await Process.run(
          'dart',
          ['run', 'runner.dart'],
          environment: {'SERVERPOD_SERVICE_SECRET': serviceSecret},
          workingDirectory: 'test_integration/assets/load_passwords',
        );
        stderr.writeln(result.stderr);
      });
      ...
      test('then the service secret is set from env', () {
        expect(result.stdout, contains('serviceSecret: $serviceSecret'));
      });
    },
  );
  ...
  group(
    ...
    () {
      ...
      test('then the service secret is not set', () {
        expect(result.stdout, isNot(contains('serviceSecret')));
      });
    },
  );
