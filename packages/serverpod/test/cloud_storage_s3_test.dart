...
void main() {
  ...
  group('Database cloud storage', () {
    ...
    test('Store file 1', () async {
      await client.s3CloudStorage.storePublicFile('testdir/myfile1.bin', createByteData(256));
    });

    test('Store file 2', () async {
      await client.s3CloudStorage.storePublicFile('testdir/myfile2.bin', createByteData(256));
    });

    test('Replace file 1', () async {
      await client.s3CloudStorage.storePublicFile('testdir/myfile1.bin', createByteData(128));
    });

    test('Retrieve file 1', () async {
      var byteData = await client.s3CloudStorage.retrievePublicFile('testdir/myfile1.bin');
  ...
  //
    test('Retrieve file 1 URL', () async {
      var urlStr = await client.s3CloudStorage.getPublicUrlForFile('testdir/myfile1.bin');
      expect(urlStr, isNotNull);
    });

    test('Retrieve file 2 through fetched URL', () async {
      var urlStr = await client.s3CloudStorage.getPublicUrlForFile('testdir/myfile2.bin');
    ...
    test('Retrieve file 2', () async {
      var byteData = await client.s3CloudStorage.retrievePublicFile('testdir/myfile2.bin');
      expect(byteData!.lengthInBytes, equals(256));
      expect(verifyByteData(byteData), equals(true));
    });

    test('Retrieve non existing file', () async {
      var byteData = await client.s3CloudStorage.retrievePublicFile('testdir/myfile3.bin');
  ...
  //
    ...
    test('Exists non existing file', () async {
      var exists = await client.s3CloudStorage.existsPublicFile('testdir/myfile3.bin');
      expect(exists, false);
    });
