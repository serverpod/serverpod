...
void main() {
  ...
  const staticCacheBustingTestName =
      'Given a StaticRoute.withCacheBusting without a Dart factory and a static cache control environment variable, '
      'when a file is requested, '
      'then the environment cache control replaces the 1 year default.';
  test(staticCacheBustingTestName, () async {
    if (await _rerunWithCacheControlEnvironment(
      testName: staticCacheBustingTestName,
      variable: _staticCacheControlVariable,
      value: 'public, max-age=60, s-maxage=86400',
    )) {
      return;
    ...
    server.pod.webServer.addRoute(
      StaticRoute.withCacheBusting(
        CacheBustingConfig(
          mountPrefix: '/static',
          fileSystemRoot: server.directory,
        ),
      ),
      '/static',
    );
    ...
    expect(response.statusCode, 200);
    expect(response.headers['cache-control'], contains('max-age=60'));
    expect(response.headers['cache-control'], isNot(contains('immutable')));
  });
  ...
  const staticEnvironmentOnFlutterAssetTestName =
      ...
      'when an asset is requested, '
      'then the private no-cache default is applied.';
  ...
  test(
    'Given a StaticRoute.withCacheBusting without a Dart factory and no cache control environment variable, '
    'when a file is requested, '
    'then the public immutable 1 year default is applied.',
    () async {
      final server = await _createFileServer();
      server.pod.webServer.addRoute(
        StaticRoute.withCacheBusting(
