...
void main() {
  ...
  group('Given table relation with relation definition', () {
    ...
    test(
      ...
      () {
        expect(tableRelation.relationQueryAlias, 'company_ceo_citizen');
      },
    );

    test('when fieldTable is called then last field table is returned.', () {
      expect(tableRelation.fieldTable, table);
    });

    test(
      'when fieldColumn is called then last joining column is returned.',
      () {
        expect(tableRelation.fieldColumn, lastJoiningColumn);
      },
    );
    ...
    test(
      ...
      () {
        expect(tableRelation.fieldQueryAlias, 'company.ceoId');
      },
    );
    ...
    group('when using copyAndAppend to create new table relation ', () {
      ...
      test('then new table relation still starts with base table name.', () {
        expect(newTableRelation.relationQueryAlias, startsWith('company'));
      });
  ...
  group('Given table relation with multiple relation definitions', () {
    ...
    test(
      'when fieldQueryAlias is called then the unescaped version of the base query for field is created.',
      () {
        expect(tableRelation.fieldQueryAlias, 'citizen.favoriteRestaurantId');
      },
    );
    ...
    test(
      ...
      () {
        ...
        expect(
          lastRelation.foreignFieldNameWithJoins,
  ...
  group(
    ...
    () {
      ...
      var companyTable = Table<int?>(tableName: companyTableName);
      ...
      var relationTable = createRelationTable(
        ...
        createTable:
            (foreignTableRelation) => Table<int?>(
