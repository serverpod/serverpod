...
void main() {
  group('Given a comprehensive RuntimeParameters implementation', () {
    ...
    group('with boolean conversion', () {
      test('when boolean is true then on is used in SQL.', () {
        var options = const _ComprehensiveRuntimeParameters(booleanTrue: true);
  ...
  group('Given a IvfflatIndexQueryOptions', () {
    ...
    test(
      'when build is called with isLocal true then LOCAL is included in SQL statements.',
      () {
        var options = const IvfflatIndexQueryOptions(probes: 3, maxProbes: 15);
  ...
  group('Given a SearchPathsConfig', () {
    ...
    test(
      ...
      () {
        var options = const SearchPathsConfig(searchPaths: null);

        var result = options.build(isLocal: false);

        expect(result, 'SET search_path TO DEFAULT;');
      },
    );
    ...
    test(
      ...
      () {
        ...
        expect(result, "SET LOCAL search_path TO 'public', 'custom_schema';");
      },
    );
    ...
    test(
      ...
      () {
        ...
        expect(result, 'SET LOCAL search_path TO DEFAULT;');
      },
    );
    ...
    test(
      ...
      () {
        var options = SearchPathsConfig(searchPaths: ['public']);
  ...
  group('Given a MapRuntimeParameters', () {
    ...
    test('when buildCheckValues is called then SELECT statement is correct.', () {
      var params = MapRuntimeParameters({'a': 1, 'b': 'x'});
