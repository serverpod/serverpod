...
class FutureCallEntryRepository {
  ...
  /// Emits [FutureCallEntry]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `FutureCallEntry.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<FutureCallEntry>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<FutureCallEntryTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<FutureCallEntry>(
      where: where?.call(FutureCallEntry.t),
      orderBy: orderBy?.call(FutureCallEntry.t),
      orderByList: orderByList?.call(FutureCallEntry.t),
      limit: limit,
