...
class FutureCallClaimEntryRepository {
  ...
  /// Emits [FutureCallClaimEntry]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `FutureCallClaimEntry.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<FutureCallClaimEntry>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<FutureCallClaimEntryTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<FutureCallClaimEntry>(
      where: where?.call(FutureCallClaimEntry.t),
      orderBy: orderBy?.call(FutureCallClaimEntry.t),
      orderByList: orderByList?.call(FutureCallClaimEntry.t),
      limit: limit,
