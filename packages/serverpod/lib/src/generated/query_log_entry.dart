...
class QueryLogEntryRepository {
  ...
  /// Emits [QueryLogEntry]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `QueryLogEntry.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<QueryLogEntry>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<QueryLogEntryTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<QueryLogEntry>(
      where: where?.call(QueryLogEntry.t),
      orderBy: orderBy?.call(QueryLogEntry.t),
      orderByList: orderByList?.call(QueryLogEntry.t),
      limit: limit,
