...
class ServerHealthMetricRepository {
  ...
  /// Emits [ServerHealthMetric]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ServerHealthMetric.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ServerHealthMetric>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ServerHealthMetricTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ServerHealthMetric>(
      where: where?.call(ServerHealthMetric.t),
      orderBy: orderBy?.call(ServerHealthMetric.t),
      orderByList: orderByList?.call(ServerHealthMetric.t),
      limit: limit,
