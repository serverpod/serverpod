...
abstract class SessionLogEntry
  ...
  SessionLogEntry copyWith({
    ...
    DateTime? touched,
    List<_iv7ld46g.LogEntry>? logs =
        const _is.$UndefinedList<_iv7ld46g.LogEntry>(),
    List<_inqjskye.QueryLogEntry>? queries =
        const _is.$UndefinedList<_inqjskye.QueryLogEntry>(),
    List<_iky1nb92.MessageLogEntry>? messages =
        const _is.$UndefinedList<_iky1nb92.MessageLogEntry>(),
  });
  @override
...
class _SessionLogEntryImpl extends SessionLogEntry {
  ...
  SessionLogEntry copyWith({
    ...
    return SessionLogEntry(
      ...
      logs: logs is _is.UndefinedSentinel
          ? this.logs?.map((e0) => e0.copyWith()).toList()
          : logs,
      queries: queries is _is.UndefinedSentinel
          ? this.queries?.map((e0) => e0.copyWith()).toList()
          : queries,
      messages: messages is _is.UndefinedSentinel
...
class SessionLogEntryRepository {
  ...
  /// Emits [SessionLogEntry]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `SessionLogEntry.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<SessionLogEntry>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<SessionLogEntryTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<SessionLogEntry>(
      where: where?.call(SessionLogEntry.t),
      orderBy: orderBy?.call(SessionLogEntry.t),
      orderByList: orderByList?.call(SessionLogEntry.t),
      limit: limit,
