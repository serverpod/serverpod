...
class LogEntryRepository {
  ...
  /// Emits [LogEntry]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `LogEntry.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<LogEntry>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<LogEntryTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<LogEntry>(
      where: where?.call(LogEntry.t),
      orderBy: orderBy?.call(LogEntry.t),
      orderByList: orderByList?.call(LogEntry.t),
      limit: limit,
