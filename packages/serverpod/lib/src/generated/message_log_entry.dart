...
class MessageLogEntryRepository {
  ...
  /// Emits [MessageLogEntry]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `MessageLogEntry.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<MessageLogEntry>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<MessageLogEntryTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<MessageLogEntry>(
      where: where?.call(MessageLogEntry.t),
      orderBy: orderBy?.call(MessageLogEntry.t),
      orderByList: orderByList?.call(MessageLogEntry.t),
      limit: limit,
