...
class ReadWriteTestEntryRepository {
  ...
  /// Emits [ReadWriteTestEntry]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ReadWriteTestEntry.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ReadWriteTestEntry>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ReadWriteTestEntryTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ReadWriteTestEntry>(
      where: where?.call(ReadWriteTestEntry.t),
      orderBy: orderBy?.call(ReadWriteTestEntry.t),
      orderByList: orderByList?.call(ReadWriteTestEntry.t),
      limit: limit,
