...
/// The TCP password [EmbeddedPostgres.start] generated for a caller who passed
/// none. A password the caller passes is never written here.
const _generatedPasswordFileName = 'postgres.password';
...
class EmbeddedPostgresImpl extends EmbeddedPostgres {
  ...
  static Future<EmbeddedPostgres> attach(
    ...
    var state = EmbeddedPostgresState.read(
      stateFile,
      tcpPassword:
          password ??
          _readIfExists(
            File(p.join(dataRoot.path, _generatedPasswordFileName)),
          ),
    );
    if (state == null) {
      throw const AttachException(
  ...
  static Future<EmbeddedPostgres> _startLocked({
    ...
    var passwordFile = File(p.join(dataRoot.path, _generatedPasswordFileName));
    ...
    var resolvedTransport = await _resolveTransport(
      options.transport,
      generatedPassword: _readIfExists(passwordFile),
    );
    ...
    // No postmaster reads the password file now, so it can follow this launch.
    var generated = options.transport.password == null
        ? resolvedTransport.password
        : null;
    if (generated != null) {
      passwordFile.writeAsStringSync(generated);
    } else {
...
///
/// A missing password becomes [generatedPassword], or a new random one.
Future<Transport> _resolveTransport(
  Transport requested, {
  ...
  return requested.withTcp(
    ...
    password: requested.password ?? generatedPassword ?? generateRandomString(),
  );
}
