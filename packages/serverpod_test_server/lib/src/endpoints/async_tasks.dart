...
class AsyncTasksEndpoint extends Endpoint {
  Future<void> insertRowToSimpleDataAfterDelay(Session session, int num, int seconds) async {
    // No await, method will return immediately and execute task
    _insertRowToSimpleDataAfterDelay(session, num, seconds);
  }

  Future<void> _insertRowToSimpleDataAfterDelay(Session session, int num, int seconds) async {
