...
class GeographyPoint {
  ...
  factory GeographyPoint.fromJson(Map<String, dynamic> json) {
    ...
    return GeographyPoint((lonVal as num).toDouble(), (latVal as num).toDouble(), srid: json['srid'] as int?);
  }

  Map<String, dynamic> toJson() => {'longitude': longitude, 'latitude': latitude, if (srid != null) 'srid': srid};
  ...
  factory GeographyPoint.fromWkt(String wkt) {
    ...
    final pointMatch = RegExp(r'^POINT\s*\(([^)]+)\)', caseSensitive: false).firstMatch(s);
    if (pointMatch == null) {
    ...
    if (coords.length < 2) throw FormatException('Invalid POINT coordinates: ${pointMatch.group(1)}');
  ...
  GeographyPoint copyWith({double? longitude, double? latitude, int? srid}) {
    return GeographyPoint(longitude ?? this.longitude, latitude ?? this.latitude, srid: srid ?? this.srid);
  }

  @override
  bool operator ==(Object other) => other is GeographyPoint && other.longitude == longitude && other.latitude == latitude && other.srid == srid;
