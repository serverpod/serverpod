...
void main() {
  group('Given a request constructor', () {
    ...
    test('when a non-default protocolVersion is provided then it is set correctly', () {
    ...
    group('Given a request URL', () {
      test("when no url is provided then it defaults to the requestedUri's relativized path and query", () {
      ...
      test('when the URL contains a colon in the first segment then it is handled correctly', () {
        var request = Request(Method.get, Uri.parse('http://localhost/foo:bar/42'));
        expect(request.url, equals(Uri.parse('foo%3Abar/42')));
      });

      test('when the URL contains a slash then it is handled correctly', () {
        var request = Request(Method.get, Uri.parse('http://localhost/foo/bar%2f42'));
        expect(request.url, equals(Uri.parse('foo/bar%2f42')));
      });

      test('when handlerPath is provided then url is inferred from it', () {
        var request = Request(Method.get, Uri.parse('http://localhost/foo/bar?q=1'), handlerPath: '/foo/');
        expect(request.url, equals(Uri.parse('bar?q=1')));
      });

      test('when a specific url is provided then it is used', () {
        var request = Request(Method.get, Uri.parse('http://localhost/foo/bar?q=1'), url: Uri.parse('bar?q=1'));
        expect(request.url, equals(Uri.parse('bar?q=1')));
      });

      test('when an empty url is provided then it is handled correctly', () {
        var request = Request(Method.get, Uri.parse('http://localhost/foo/bar'), url: Uri.parse(''));
    ...
    group('Given a request handlerPath', () {
      ...
      test('when a handlerPath without a trailing slash is provided then it adds a trailing slash', () {
        var request = Request(Method.get, Uri.parse('http://localhost/foo/bar?q=1'), handlerPath: '/foo');
        expect(request.handlerPath, equals('/foo/'));
        expect(request.url, equals(Uri.parse('bar?q=1')));
      });

      test('when a single slash is provided as handlerPath then it is handled correctly', () {
        var request = Request(Method.get, Uri.parse('http://localhost/foo/bar?q=1'), handlerPath: '/');
        expect(request.handlerPath, equals('/'));
        expect(request.url, equals(Uri.parse('foo/bar?q=1')));
      });
    });
    ...
    group('Given request errors', () {
      ...
      group('Given a url', () {
        test('when it is not relative then it throws an ArgumentError', () {
          expect(() {
            Request(Method.get, Uri.parse('http://localhost/test'), url: Uri.parse('http://localhost/test'));
          }, throwsArgumentError);
        });

        test('when it is root-relative then it throws an ArgumentError', () {
          expect(() {
            Request(Method.get, Uri.parse('http://localhost/test'), url: Uri.parse('/test'));
          }, throwsArgumentError);
        });

        test('when it has a fragment then it throws an ArgumentError', () {
          expect(() {
            Request(Method.get, Uri.parse('http://localhost/test'), url: Uri.parse('test#fragment'));
          }, throwsArgumentError);
        });

        test('when it is not a suffix of requestedUri then it throws an ArgumentError', () {
          expect(() {
            Request(Method.get, Uri.parse('http://localhost/dir/test'), url: Uri.parse('dir'));
          }, throwsArgumentError);
        });

        test('when it does not have the same query parameters as requestedUri then it throws an ArgumentError', () {
          expect(() {
            Request(Method.get, Uri.parse('http://localhost/test?q=1&r=2'), url: Uri.parse('test?q=2&r=1'));
          }, throwsArgumentError);

          // Order matters for query parameters.
          expect(() {
            Request(Method.get, Uri.parse('http://localhost/test?q=1&r=2'), url: Uri.parse('test?r=2&q=1'));
          }, throwsArgumentError);
        });
      });

      group('Given a handlerPath', () {
        test('when it is not a prefix of requestedUri then it throws an ArgumentError', () {
          expect(() {
            Request(Method.get, Uri.parse('http://localhost/dir/test'), handlerPath: '/test');
          }, throwsArgumentError);
        });

        test('when it does not start with "/" then it throws an ArgumentError', () {
          expect(() {
            Request(Method.get, Uri.parse('http://localhost/test'), handlerPath: 'test');
          }, throwsArgumentError);
        });

        test('when url is empty and handlerPath is not the requestedUri path then it throws an ArgumentError', () {
          expect(() {
            Request(Method.get, Uri.parse('http://localhost/test'), handlerPath: '/', url: Uri.parse(''));
          }, throwsArgumentError);
        });
      });

      group('Given handlerPath + url', () {
        test('when they do not form the requestedUrl path then it throws an ArgumentError', () {
          expect(() {
            Request(Method.get, Uri.parse('http://localhost/foo/bar/baz'), handlerPath: '/foo/', url: Uri.parse('baz'));
          }, throwsArgumentError);
        });

        test('when they are not on a path boundary then it throws an ArgumentError', () {
          expect(() {
            Request(Method.get, Uri.parse('http://localhost/foo/bar/baz'), handlerPath: '/foo/ba', url: Uri.parse('r/baz'));
          }, throwsArgumentError);
        });
      });
    });
  ...
  group('Given ifModifiedSince', () {
    ...
    test('when there is a Last-Modified header then it is set correctly', () {
      var request = _request(headers: Headers.request(ifModifiedSince: parseHttpDate('Sun, 06 Nov 1994 08:49:37 GMT')));
      expect(request.headers.ifModifiedSince, equals(DateTime.parse('1994-11-06 08:49:37z')));
    });
  });
  ...
  group('Given a request change', () {
    ...
    group('Given a path', () {
      test('when updated then it updates handlerPath and url', () {
        ...
        var request = Request(Method.get, uri, handlerPath: '/static/', url: Uri.parse('dir/file.html'));
      ...
      test('when handling a regression test for Issue #142 then it is handled correctly', () {
        var uri = Uri.parse('https://test.example.com/static/dir/');
        var request = Request(Method.get, uri, handlerPath: '/static/', url: Uri.parse('dir/'));
      ...
      test('when changing path leading to double // then it is handled correctly', () {
        var uri = Uri.parse('https://test.example.com/some_base//more');
        var request = Request(Method.get, uri, handlerPath: '', url: Uri.parse('some_base//more'));
      ...
      test('when path does not match existing uri then it throws an ArgumentError', () {
        var uri = Uri.parse('https://test.example.com/static/dir/file.html');
        var request = Request(Method.get, uri, handlerPath: '/static/', url: Uri.parse('dir/file.html'));
