...
void main() {
  group('Given a cascade with several handlers', () {
    ...
    test('when a request with no headers is processed then the first response should be returned', () async {
      var response = await makeSimpleRequest(handler);
      expect(response.statusCode, equals(200));
      expect(response.readAsString(), completion(equals('handler 1')));
    });

    test('when a request with header "one: false" is processed then the second response should be returned', () async {
    ...
    test('when a request with headers "one: false" and "two: false" is processed then the third response should be returned', () async {
      final response = await handler(
        Request(
          Method.get,
          localhostUri,
    ...
    test('when a request with headers "one: false", "two: false", and "three: false" is processed then a 404 response should be returned', () async {
      final response = await handler(
        Request(
          Method.get,
          localhostUri,
  ...
  test('Given a cascade with a 404 response when processed then it triggers the next handler', () async {
    var handler = Cascade()
  ...
  test('Given a cascade with a 405 response when processed then it triggers the next handler', () async {
    var handler = Cascade()
        .add((_) => Response(405))
        .add((_) => Response.ok(body: Body.fromString('handler 2')))
        .handler;
  ...
  test('Given a cascade with specific statusCodes when processed then it controls which statuses cause cascading', () async {
  ...
  test('Given a cascade with shouldCascade when processed then it controls which responses cause cascading', () async {
    var handler =
  ...
  group('Given error scenarios', () {
    test('when getting the handler for an empty cascade then it throws a StateError', () {
      expect(() => Cascade().handler, throwsStateError);
    });

    test('when both statusCodes and shouldCascade are provided then it throws an ArgumentError', () {
      expect(
