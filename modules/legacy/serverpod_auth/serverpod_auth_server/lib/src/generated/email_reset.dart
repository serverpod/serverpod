...
class EmailResetRepository {
  ...
  /// Emits [EmailReset]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `EmailReset.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<EmailReset>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<EmailResetTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<EmailReset>(
      where: where?.call(EmailReset.t),
      orderBy: orderBy?.call(EmailReset.t),
      orderByList: orderByList?.call(EmailReset.t),
      limit: limit,
