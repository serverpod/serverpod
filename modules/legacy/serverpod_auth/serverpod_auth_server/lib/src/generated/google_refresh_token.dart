...
class GoogleRefreshTokenRepository {
  ...
  /// Emits [GoogleRefreshToken]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `GoogleRefreshToken.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<GoogleRefreshToken>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<GoogleRefreshTokenTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<GoogleRefreshToken>(
      where: where?.call(GoogleRefreshToken.t),
      orderBy: orderBy?.call(GoogleRefreshToken.t),
      orderByList: orderByList?.call(GoogleRefreshToken.t),
      limit: limit,
