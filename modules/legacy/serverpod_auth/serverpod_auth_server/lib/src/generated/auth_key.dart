...
class AuthKeyRepository {
  ...
  /// Emits [AuthKey]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `AuthKey.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<AuthKey>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<AuthKeyTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<AuthKey>(
      where: where?.call(AuthKey.t),
      orderBy: orderBy?.call(AuthKey.t),
      orderByList: orderByList?.call(AuthKey.t),
      limit: limit,
