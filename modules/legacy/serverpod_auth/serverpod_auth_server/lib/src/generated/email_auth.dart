...
class EmailAuthRepository {
  ...
  /// Emits [EmailAuth]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `EmailAuth.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<EmailAuth>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<EmailAuthTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<EmailAuth>(
      where: where?.call(EmailAuth.t),
      orderBy: orderBy?.call(EmailAuth.t),
      orderByList: orderByList?.call(EmailAuth.t),
      limit: limit,
