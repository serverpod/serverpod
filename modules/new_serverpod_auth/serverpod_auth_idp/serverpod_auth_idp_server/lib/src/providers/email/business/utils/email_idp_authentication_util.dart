    ...
    if (!await _hashUtil.validateHash(
      ...
      onFallbackValidated: (final newHash) async {
        ...
        await EmailAccount.db.updateRow(
          session,
          account.copyWith(
            passwordHash: newHash.hash.asByteData,
            passwordSalt: newHash.salt.asByteData,
          ),
          // Not using transaction here to ensure the rehash is persisted
          // even if the outer transaction is rolled back
        );
      },
    )) {
      await _logFailedSignIn(session, email);
