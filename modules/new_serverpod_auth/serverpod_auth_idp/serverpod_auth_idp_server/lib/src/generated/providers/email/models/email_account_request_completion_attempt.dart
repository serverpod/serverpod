...
class EmailAccountRequestCompletionAttemptRepository {
  ...
  /// Updates a single [EmailAccountRequestCompletionAttempt] by its [id] with the specified [columns].
  /// Returns the updated row or null if no row with the given id exists.
  Future<EmailAccountRequestCompletionAttempt?> updateById(
    _i1.Session session,
    _i1.UuidValue id,
    _i1.ColumnValueListBuilder<EmailAccountRequestCompletionAttemptTable>
        columns, {
    _i1.Transaction? transaction,
  }) async {
    return session.db.updateById<EmailAccountRequestCompletionAttempt>(
      id,
      columns(EmailAccountRequestCompletionAttempt.t),
      transaction: transaction,
    );
  }

  /// Updates all [EmailAccountRequestCompletionAttempt]s matching the [where] expression with the specified [columns].
