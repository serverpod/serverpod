...
void main() {
  group('Given a valid HS512 configuration and', () {
    ...
    group('a plain refresh token,', () {
      ...
      test(
        ...
        () {
          expect(jwtUtil.createJwt(refreshToken), isNotEmpty);
        },
      );
    ...
    group('a refresh token containing a reserved claim,', () {
      ...
      test(
        ...
        () {
          expect(() => jwtUtil.createJwt(refreshToken), throwsArgumentError);
        },
      );
    });
    ...
    group('a JWT token for a plain refresh token,', () {
      ...
      test(
        ...
        () {
          expect(JWT.decode(jwt).subject, refreshToken.authUserId.toString());
        },
      );
      ...
      test(
        ...
        () {
          expect(JWT.decode(jwt).issuer, isNull);
        },
      );
      ...
      test('when the JWT is inspected, then its `extraClaims` are empty.', () {
        ...
        expect(tokenData.extraClaims, isEmpty);
      });
      ...
      group('an access token for a refresh token with extra claims defined,', () {
        ...
        test(
          ...
          () {
            ...
            expect(tokenData.extraClaims, {'b': 1, 'a': 'test'});
          },
        );
      });
    });
  ...
  test(
    ...
    () {
      ...
      expect(JWT.decode(jwt).issuer, issuer);
    },
  );
  ...
  test(
    ...
    () {
      ...
      final differentIssuerHS512Util =
          AuthenticationTokens(
            config: AuthenticationTokenConfig(
              algorithm: _hs512Algorithm(),
              refreshTokenHashPepper: 'test-pepper',
  ...
  group(
    ...
    () {
      ...
      test('when a JWT is verified, then its data is returned.', () {
        expect(jwtUtil.verifyJwt(jwt), isNotNull);
      });

      test('when the JWT is decoded, then it names HS512 as its `alg.', () {
        expect(JWT.decode(jwt).header, equals({'alg': 'ES512', 'typ': 'JWT'}));
      });
      ...
      test('when validated by HMAC, then the validation fails.', () {
        ...
        expectLater(() => hmacJwtUtil.verifyJwt(jwt), throwsA(isA<Error>()));
      });
