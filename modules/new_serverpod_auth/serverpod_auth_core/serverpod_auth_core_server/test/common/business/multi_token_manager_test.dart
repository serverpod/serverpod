...
void main() {
  withServerpod('Given a MultiTokenManager with multiple fake managers', (
    ...
    setUp(() {
      ...
      multiTokenManager = MultiTokenManager(
        ...
        additionalTokenManagers: [additionalManager1, additionalManager2],
      );
    });
  ...
  withServerpod('Given tokens in multiple managers', (
    final sessionBuilder,
    final endpoints,
  ) {
    late Session session;
    ...
    setUp(() async {
      ...
      defaultManager = FakeTokenManager(defaultStorage, tokenIssuer: 'default');
      ...
      final additionalManager1AuthSuccess = await additionalManager1.issueToken(
        session,
        authUserId: user1Id,
        method: 'oauth',
      ...
      additionalManager1TokenId =
          (await additionalManager1.listTokens(
            session,
            authUserId: user1Id,
          )).single.tokenId;
    ...
    group('when listing tokens by method', () {
      ...
      test('then only tokens with specified method should be returned', () {
        ...
        expect(emailTokens.every((final t) => t.method == 'email'), isTrue);
      });
    ...
    test(
      ...
      () async {
        expect(
          () => multiTokenManager.revokeToken(session, tokenId: 'non-existent'),
          returnsNormally,
        );
      },
    );
    ...
    group('when revoking all tokens with combined filters', () {
      ...
      test('then only matching tokens should be revoked', () async {
        ...
        expect(
          remainingTokens.any(
            (final t) => t.userId == user1Id.toString() && t.method == 'email',
          ),
          isFalse,
    ...
    group('when validating a token from the default manager', () {
      ...
      test('then the scopes should match', () {
        expect(authInfo!.scopes.map((final s) => s.name), contains('read'));
      });
    });
    ...
    test(
      ...
      () async {
        ...
        expect(authInfo, isNotNull);
        expect(authInfo!.userIdentifier, equals(user1Id.toString()));
        expect(authInfo.scopes.map((final s) => s.name), contains('write'));
      },
    );
    ...
    test(
      ...
      () async {
        ...
        expect(authInfo.scopes.map((final s) => s.name), contains('admin'));
      },
    );
      ...
      setUp(() {
        ...
        final primaryTokenManager = MockedTokenManager(
          onValidateToken: (final session, final token) async {
            return AuthenticationInfo(primaryTokenManagerUser, {
              const Scope('read'),
            }, authId: 'token1');
          },
        );
        final additionalTokenManager = MockedTokenManager(
          onValidateToken: (final session, final token) async {
            return AuthenticationInfo('additional-token-manager-user', {
              const Scope('write'),
            }, authId: 'token2');
          },
        );
        multiTokenManager = MultiTokenManager(
          ...
          additionalTokenManagers: [additionalTokenManager],
        );
      });
    ...
    (final sessionBuilder, final endpoints) {
      ...
      setUp(() {
        ...
        final firstAdditionalTokenManager = MockedTokenManager(
          onValidateToken: (final session, final token) async {
            return AuthenticationInfo(firstAdditionalTokenManagerUser, {
              const Scope('write'),
            }, authId: 'token2');
          },
        );
        final secondAdditionalTokenManager = MockedTokenManager(
          onValidateToken: (final session, final token) async {
            return AuthenticationInfo('second-additional-token-manager-user', {
              const Scope('read'),
            }, authId: 'token3');
          },
        );
  ...
  withServerpod('Given a MultiTokenManager with only a default manager', (
    final sessionBuilder,
    final endpoints,
  ) {
    late Session session;
  ...
  withServerpod('Given a MultiTokenManager with many additional managers', (
    final sessionBuilder,
    final endpoints,
  ) {
    late Session session;
  ...
  withServerpod('Given a MultiTokenManager with kind-aware managers', (
    final sessionBuilder,
    final endpoints,
  ) {
    late Session session;
    ...
    setUp(() async {
      ...
      multiTokenManager = MultiTokenManager(
        primaryTokenManager: defaultManager,
        additionalTokenManagers: [manager1, manager2, manager3],
      );
