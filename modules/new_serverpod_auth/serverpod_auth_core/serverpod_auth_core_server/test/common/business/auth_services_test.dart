...
void main() {
  withServerpod('Given AuthServices is being configured', (
    ...
    setUp(() {
      ...
      identityProviderFactories = [FakeIdentityProviderFactory()];

      tokenManagers = [FakeTokenManager(fakeTokenStorage)];
    });
  ...
  withServerpod('Given an AuthServices with identity providers', (
    final sessionBuilder,
    final endpoints,
  ) {
    late FakeTokenManager fakeTokenManager;
  ...
  withServerpod('Given an AuthServices with multiple identity providers', (
    final sessionBuilder,
    final endpoints,
  ) {
    late FakeTokenManager fakeTokenManager;
  ...
  withServerpod('Given an AuthServices with authentication handler', (
    final sessionBuilder,
    final endpoints,
  ) {
    late FakeTokenManager fakeTokenManager;
    ...
    group('when validating a valid token with single scope', () {
      ...
      setUp(() async {
        final authSuccess = await fakeTokenManager.issueToken(
          session,
        ...
        result = await authServices.authenticationHandler(session, validToken);
      });
      ...
      test('then the scopes should match', () {
        expect(result!.scopes.map((final s) => s.name), contains('test-scope'));
      });
    });
