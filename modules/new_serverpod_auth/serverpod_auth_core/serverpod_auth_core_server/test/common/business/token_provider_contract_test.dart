...
abstract interface class TestableTokenProvider implements TokenProvider {
  void testAddToken(final TokenInfo token);
  void testClear();
}

class FakeTokenProvider implements TestableTokenProvider {
  ...
  Future<void> revokeAllTokens({
    ...
    final tokensToRemove = _tokens.values.where((final token) {
      if (authUserId != null && token.userId != authUserId.toString()) {
        return false;
      }
...
void testSuite(final TestableTokenProvider Function() providerBuilder) {
  group('TokenProvider', () {
    withServerpod(
      'Given multiple tokens exist',
      (final sessionBuilder, final endpoints) {
        late TestableTokenProvider tokenProvider;
        ...
        setUp(() {
          ...
          tokenProvider.testAddToken(TokenInfo(
