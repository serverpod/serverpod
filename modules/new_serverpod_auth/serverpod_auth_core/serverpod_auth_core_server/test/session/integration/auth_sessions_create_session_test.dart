...
void main() {
  group('Given AuthSessions with default session lifetime configured', () {
    ...
    withServerpod('when creating a session without explicit expiresAt', (
      ...
      test('then session is valid before default lifetime expires.', () async {
        await withClock(
    ...
    withServerpod('when creating a session with explicit expiresAt', (
      final sessionBuilder,
      final endpoints,
    ) {
      late Session session;
      ...
      test('then explicit expiresAt takes precedence over default.', () async {
  ...
  group('Given AuthSessions with no session configuration defaults', () {
    ...
    withServerpod('when creating a session', (
      final sessionBuilder,
      final endpoints,
    ) {
      late Session session;
