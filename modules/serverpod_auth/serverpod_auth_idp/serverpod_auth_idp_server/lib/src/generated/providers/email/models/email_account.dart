...
class EmailAccountRepository {
  ...
  /// Emits [EmailAccount]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `EmailAccount.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<EmailAccount>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<EmailAccountTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<EmailAccount>(
      where: where?.call(EmailAccount.t),
      orderBy: orderBy?.call(EmailAccount.t),
      orderByList: orderByList?.call(EmailAccount.t),
      limit: limit,
