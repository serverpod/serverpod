...
class GoogleAccountRepository {
  ...
  /// Emits [GoogleAccount]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `GoogleAccount.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<GoogleAccount>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<GoogleAccountTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<GoogleAccount>(
      where: where?.call(GoogleAccount.t),
      orderBy: orderBy?.call(GoogleAccount.t),
      orderByList: orderByList?.call(GoogleAccount.t),
      limit: limit,
