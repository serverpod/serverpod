...
abstract class MicrosoftAccount
  ...
  MicrosoftAccount copyWith({
    ...
    _is.UuidValue? authUserId,
    _iacs.AuthUser? authUser = const _UndefinedMicrosoftAccount$authUser(),
    String? userIdentifier,
    String? email,
    DateTime? created,
  });
...
class _UndefinedMicrosoftAccount$authUser extends _issu.UndefinedSentinel
    implements _iacs.AuthUser {
  const _UndefinedMicrosoftAccount$authUser();
}
...
class MicrosoftAccountRepository {
  ...
  /// Emits [MicrosoftAccount]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `MicrosoftAccount.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<MicrosoftAccount>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<MicrosoftAccountTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<MicrosoftAccount>(
      where: where?.call(MicrosoftAccount.t),
      orderBy: orderBy?.call(MicrosoftAccount.t),
      orderByList: orderByList?.call(MicrosoftAccount.t),
      limit: limit,
