...
class _UndefinedGitHubAccount$authUser extends _issu.UndefinedSentinel
...
class GitHubAccountRepository {
  ...
  /// Emits [GitHubAccount]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `GitHubAccount.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<GitHubAccount>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<GitHubAccountTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<GitHubAccount>(
      where: where?.call(GitHubAccount.t),
      orderBy: orderBy?.call(GitHubAccount.t),
      orderByList: orderByList?.call(GitHubAccount.t),
      limit: limit,
