...
class FirebaseAccountRepository {
  ...
  /// Emits [FirebaseAccount]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `FirebaseAccount.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<FirebaseAccount>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<FirebaseAccountTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<FirebaseAccount>(
      where: where?.call(FirebaseAccount.t),
      orderBy: orderBy?.call(FirebaseAccount.t),
      orderByList: orderByList?.call(FirebaseAccount.t),
      limit: limit,
