...
class Users {
  ...
  static Future<UserInfo?> changeUserName(Session session, int userId, String newUserName) async {
  ...
  static Future<UserInfo?> updateUserScopes(Session session, int userId, Set<Scope> newScopes) async {
    var userInfo = await findUserByUserId(session, userId, useCache: false);
    if (userInfo == null)
      return null;
    ...
    await session.db.query('UPDATE serverpod_auth_key SET "scopeNames"=\'$json\' WHERE "userId" = $userId');
