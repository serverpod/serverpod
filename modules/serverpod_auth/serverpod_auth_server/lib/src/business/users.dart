...
class Users {
  ...
  static Future<UserInfo?> createUser(
    ...
    if (AuthConfig.current.onUserWillBeCreated != null) {
      bool approved = await AuthConfig.current.onUserWillBeCreated!(
        session,
        userInfo,
        authMethod,
      );
  ...
  static Future<UserInfo?> findUserByUserId(Session session, int userId,
    ...
    String cacheKey = 'serverpod_auth_userinfo_$userId';
    UserInfo? userInfo;
  ...
  static Future<UserInfo?> updateUserScopes(
    ...
    String json = jsonEncode(scopeStrs);
