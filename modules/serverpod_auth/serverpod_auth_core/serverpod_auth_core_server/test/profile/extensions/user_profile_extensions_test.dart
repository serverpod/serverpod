...
void main() {
  test(
    'Given a target profile with null fields, '
    'when merging with a source profile having values, '
  ...
  test(
    'Given a target profile with empty string fields, '
    'when merging with a source profile having values, '
    'then the fields are populated.',
    () {
      final target = UserProfile(
  ...
  test(
    'Given a target profile with populated fields, '
    'when merging with a source profile, '
    'then the target fields are preserved.',
    () {
  ...
  test(
    'Given a target profile with some populated and some missing fields, '
    'when merging with a populated source profile, '
    'then only the missing target fields are filled.',
    () {
      final target = UserProfile(
        authUserId: uuid.v4obj(),
        userName: 'target_user',
  ...
  test(
    'Given a source profile with null or empty fields, '
    'when merging into a populated target profile, '
    'then the target fields are preserved.',
    () {
      final imageId = uuid.v4obj();
      final target = UserProfile(
