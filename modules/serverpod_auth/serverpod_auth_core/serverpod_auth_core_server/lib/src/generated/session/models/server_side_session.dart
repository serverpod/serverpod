...
class ServerSideSessionRepository {
  ...
  /// Emits [ServerSideSession]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `ServerSideSession.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<ServerSideSession>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ServerSideSessionTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<ServerSideSession>(
      where: where?.call(ServerSideSession.t),
      orderBy: orderBy?.call(ServerSideSession.t),
      orderByList: orderByList?.call(ServerSideSession.t),
      limit: limit,
