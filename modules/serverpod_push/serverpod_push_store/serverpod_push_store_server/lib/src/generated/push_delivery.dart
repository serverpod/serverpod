...
class PushDeliveryRepository {
  ...
  /// Emits [PushDelivery]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `PushDelivery.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<PushDelivery>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<PushDeliveryTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<PushDelivery>(
      where: where?.call(PushDelivery.t),
      orderBy: orderBy?.call(PushDelivery.t),
      orderByList: orderByList?.call(PushDelivery.t),
      limit: limit,
