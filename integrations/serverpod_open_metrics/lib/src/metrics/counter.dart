...
///
/// **Concurrency:** This implementation is designed for Dart's single-isolate
/// async/await concurrency model. Counter increments are safe from race
/// conditions within a single isolate because individual operations complete
/// atomically between await points. For multi-isolate deployments, each isolate
/// should maintain its own registry, or use an external metrics aggregator.
///
...
class Counter implements Metric {
  ...
  /// This should typically only be used for testing.
  void reset() {
    _value = 0;
  }
