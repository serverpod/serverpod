...
import 'package:serverpod_cloud_storage_cos/src/cos_cloud_storage.dart';
import 'package:serverpod_cloud_storage_cos/src/signing/cos_signer.dart';
import 'package:test/test.dart';

void main() {
  final hasCredentials =
      Platform.environment['SERVERPOD_TEST_COS_ACCESS_KEY_ID'] != null &&
      Platform.environment['SERVERPOD_TEST_COS_SECRET_KEY'] != null &&
      Platform.environment['SERVERPOD_TEST_COS_BUCKET'] != null &&
      Platform.environment['SERVERPOD_TEST_COS_REGION'] != null;

  final skipReason = !hasCredentials
      ? 'COS credentials not configured in environment. '
            'Set SERVERPOD_TEST_COS_ACCESS_KEY_ID, '
            'SERVERPOD_TEST_COS_SECRET_KEY, '
            'SERVERPOD_TEST_COS_BUCKET, and '
            'SERVERPOD_TEST_COS_REGION to run these tests.'
      : null;

  group('COS', skip: skipReason, () {
    ...
    setUpAll(() {
      ...
      storage = _createStorageFromEnvironment()!;
    });
    ...
    group('Given no file in the storage', () {
      test(
        ...
        () async {
          ...
          final content = 'Hello, COS! ${DateTime.now()}';
          final data = ByteData.view(
            Uint8List.fromList(content.codeUnits).buffer,
          );

          await storage.storeFile(session: session, path: path, byteData: data);
    ...
    group('Given a successfully uploaded file', () {
      test('when checking if it exists then it returns true', () async {
        ...
        final exists = await storage.fileExists(session: session, path: path);
      ...
      test('when deleting it then it no longer exists', () async {
        ...
        var exists = await storage.fileExists(session: session, path: path);
        expect(exists, isTrue);

        await storage.deleteFile(session: session, path: path);

        exists = await storage.fileExists(session: session, path: path);
    ...
    group('Given a file path that does not exist', () {
      test('when checking if it exists then it returns false', () async {
      ...
      test('when retrieving it then it returns null', () async {
        final retrieved = await storage.retrieveFile(
          session: session,
          path:
              'non-existent-file-${DateTime.now().millisecondsSinceEpoch}.txt',
    ...
    group('Given binary data to upload', () {
      test('when uploading then the exact bytes are preserved', () async {
    ...
    group('Given a valid upload description', () {
      test('when creating a direct file upload description '
          'then a non-empty string is returned', () async {
      ...
      test('when uploading a file with contentLength via FileUploader '
          'then it can be retrieved', () async {
      ...
      test('when verifying a non-existent file then it returns false', () async {
        final verified = await storage.verifyDirectFileUpload(
          session: session,
          path:
              'non-existent-file-${DateTime.now().millisecondsSinceEpoch}.txt',
      ...
      test('when uploading a binary file via FileUploader '
          'then the exact bytes are preserved', () async {
    ...
    group('Given an existing file with preventOverwrite enabled', () {
      test('when uploading via storeFileWithOptions then it throws', () async {
      ...
      test('when uploading via direct upload then it fails', () async {
    ...
    group('Given a file path with special characters', () {
      test('when the path contains spaces '
          'then the file is stored and retrieved correctly', () async {
      ...
      test('when the path contains unicode characters '
          'then the file is stored correctly', () async {
...
/// Loads COS configuration from environment variables.
///
/// Returns null if credentials are not configured, causing tests to skip.
CosCloudStorage? _createStorageFromEnvironment() {
  final accessKey = Platform.environment['SERVERPOD_TEST_COS_ACCESS_KEY_ID'];
  final secretKey = Platform.environment['SERVERPOD_TEST_COS_SECRET_KEY'];
  final bucket = Platform.environment['SERVERPOD_TEST_COS_BUCKET'];
  final region = Platform.environment['SERVERPOD_TEST_COS_REGION'];
  ...
  return CosCloudStorage.withSigner(
    storageId: 'test',
    public: false,
    bucket: bucket,
    region: region,
