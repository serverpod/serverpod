...
suite('VS Code Extension', () => {
    ...
    suite('activate', () => {
        ...
        test('Given all prerequisites are met when activate is called then the client should register all expected document selectors.', () => {
            ...
            const originalLanguageClient = LanguageClient;
            const LanguageClientSpy = function(
                id: string,
                name: string,
                serverOptions: any,
                clientOptions: any
                ...
                const instance = new originalLanguageClient(id, name, serverOptions, clientOptions);
                sinon.stub(instance, 'start');
                return instance;
            } as any;

            // Temporarily replace LanguageClient
            const LanguageClientModule = require('vscode-languageclient/node');
            const originalLC = LanguageClientModule.LanguageClient;
            LanguageClientModule.LanguageClient = LanguageClientSpy;

            activate(mockContext);

            // Restore
            LanguageClientModule.LanguageClient = originalLC;
