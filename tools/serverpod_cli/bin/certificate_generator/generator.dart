...
Future<void> performGenerateCerts(String runMode, bool verbose) async {
  ...
  for(var serverId in config.cluster.keys) {
    ...
    File(certConfigFile).writeAsStringSync('[ req ]\ndistinguished_name = req_distinguished_name\nprompt = no\n\n[ req_distinguished_name ]\nCN = $serverAddress\n');

    var args = <String>['req', '-x509', '-sha256', '-nodes', '-days', '3650', '-newkey', 'rsa:2048', '-config', certConfigFile, '-keyout', keyFileName, '-out', crtFileName];
    if (verbose)
      print('openssl ' + args.join(' '));
