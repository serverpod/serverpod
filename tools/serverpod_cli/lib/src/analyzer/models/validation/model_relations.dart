...
typedef AutoGeneratedUniqueIndex = ({
...
class ParsedModelsCollection {
  ...
  late final Map<String, AutoGeneratedUniqueIndex> autoGeneratedUniqueIndexes;
  ...
  /// marked unique in any parsed model, returns that model and the field that
  /// owns the unique modifier.
  AutoGeneratedUniqueIndex? findAutoUniqueIndexOwner(String indexName) {
    return autoGeneratedUniqueIndexes[indexName];
  }
  ...
  Map<String, AutoGeneratedUniqueIndex> _createAutoGeneratedUniqueIndexesMap(
    List<SerializableModelDefinition> models,
  ) {
    return {
      for (var m in models)
        if (m is ModelClassDefinition && m.tableName != null)
          for (var f in m.fields)
            if (f.autoGeneratedUniqueIndexDefinition(m.tableName!, m.fields)
                case var index?)
              index.name: (model: m, field: f),
    };
  }
