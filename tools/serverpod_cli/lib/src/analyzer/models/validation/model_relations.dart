...
class ParsedModelsCollection {
  ...
  late final Map<SerializableModelIndexDefinition, ModelClassDefinition>
  autoGeneratedUniqueIndexes;
  ...
  ParsedModelsCollection(
    ...
    autoGeneratedUniqueIndexes = _createAutoGeneratedUniqueIndexesMap(models);
  }

  /// If [indexName] is the auto-generated unique index name for some field
  /// with `shouldCreateUniqueIndex` in any parsed model, returns that model
  /// and field name (same naming as [ModelClassDefinition.indexesIncludingInherited]).
  ({ModelClassDefinition model, SerializableModelIndexDefinition index})?
  findAutoUniqueIndexOwner(
    String indexName,
  ) {
    var entry = autoGeneratedUniqueIndexes.entries
        .where((e) => e.key.name == indexName)
        .firstOrNull;

    if (entry == null) return null;
    return (model: entry.value, index: entry.key);
  }
  ...
  Map<SerializableModelIndexDefinition, ModelClassDefinition>
  _createAutoGeneratedUniqueIndexesMap(
    List<SerializableModelDefinition> models,
  ) {
    return {
      for (var model in models)
        if (model is ModelClassDefinition && model.tableName is String)
          for (var field in model.fields)
            ?field.autoGeneratedUniqueIndexDefinition(model.tableName!): model,
    };
  }
