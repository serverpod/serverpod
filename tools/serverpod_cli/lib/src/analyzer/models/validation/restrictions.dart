...
class Restrictions {
  ...
  List<SourceSpanSeverityException> validateAutoGeneratedUniqueIndexes() {
    var definition = documentDefinition;
    if (definition is! ModelClassDefinition || definition.tableName == null) {
      return [];
    }
    ...
    for (var field in definition.fields) {
      ...
      var fieldNode = fieldsMap?.nodes[field.name];
      var uniqueSpan = fieldNode is YamlMap
          ? fieldNode.nodes[Keyword.unique]?.span
          : fieldNode?.span;

      if (index.name.length > DatabaseConstants.pgsqlMaxNameLimitation) {
        errors.add(
          SourceSpanSeverityException(
            'The auto-generated unique index name "${index.name}" for field '
            '"${field.name}" exceeds the '
            '${DatabaseConstants.pgsqlMaxNameLimitation} character index name '
            'limitation.',
            uniqueSpan,
          ),
        );
      ...
      if (!seenIndexNames.add(index.name)) {
        errors.add(
          SourceSpanSeverityException(
            'Multiple fields generate the same auto-generated unique index '
            'name "${index.name}".',
            uniqueSpan,
          ),
        );
