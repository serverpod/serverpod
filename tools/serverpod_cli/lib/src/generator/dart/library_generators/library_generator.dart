      ...
      literalMap({
        for (var parameterDef in params)
          ...
          literalString(parameterDef.name): switch (parameterDef.type) {
            RecordTypeDefinition _ =>
              mapRecordToJsonRef.call([refer('record')]).code,
            ClassTypeDefinition classDef => classDef.returnsRecordInContainer
                ? Block.of([
                    if (classDef.nullable)
  ...
  Code prepareRecordMapForSending({
    ...
    if (keyType is ClassTypeDefinition && keyType.className == 'String') {
      return Block.of([
        ...
        mapRecordToJsonRef.call([refer('entry.value')]).code,
        const Code(','),
        const Code('}'),
      ]);
    } else {
      return Block.of([
        ...
        if (keyType is RecordTypeDefinition)
          mapRecordToJsonRef.call([refer('entry.key')]).code
        else
          const Code('entry.key'),
        const Code(','),
