...
class GenerateCommand extends ServerpodCommand<GenerateOption> {
  ...
  Future<void> runWithConfig(
    ...
    var serverPubspecFile = File(
      path.joinAll([...config.serverPackageDirectoryPathParts, 'pubspec.yaml']),
    );
    var clientPubspecFile = File(
      path.joinAll([...config.clientPackagePathParts, 'pubspec.yaml']),
    );
    var pubspecsToCheck = [
      serverPubspecFile,
    ...
    if (warnings.isEmpty) {
      ...
      var clientLockFile = File(
        path.joinAll([...config.clientPackagePathParts, 'pubspec.lock']),
      );
