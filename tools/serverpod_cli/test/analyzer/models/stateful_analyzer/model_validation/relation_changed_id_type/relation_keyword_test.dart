...
void main() {
  for (var idType in SupportedIdType.all) {
    ...
    group(
        'Given a class with a self relation on a field with the class datatype',
        () {
      var models = [
      ...
      group('', () {
        ...
        test(
          ...
          expect(
              (relation as ForeignRelationDefinition).parentTable, 'example');
        });
    ...
    group('Given a class with a field with a self relation', () {
      var models = [
        ...
        table: example
        fields:
          parentId: $idClassName, relation(parent=example)
        ''',
        ).build()
      ];
    ...
    test(
      ...
      var models = [
        ...
        table: example
        fields:
          parentId: $idClassName, relation(parent=example, parent=example)
        ''',
        ).build()
      ];
...
fields:
  parentId: $idClassName, relation(parent=example, parent=example)
        ...
        var expectedStartColumn = 39 + idClassName.length;
        expect(startSpan.line, 3);
        expect(startSpan.column, expectedStartColumn);

        var endSpan = error.span!.end;
        expect(endSpan.line, 3);
        expect(endSpan.column, expectedStartColumn + 'parent'.length);
      },
    );
    ...
    test(
      ...
      var models = [
        ...
        table: example
        fields:
          parent: $idClassName, relation(optional, parent=example)
        ''',
        ).build()
      ];
      var collector = CodeGenerationCollector();
