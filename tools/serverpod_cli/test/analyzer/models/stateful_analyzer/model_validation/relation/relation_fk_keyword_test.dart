...
void main() {
      ...
      test(
        ...
        () {
          expect(
            ...
            contains(
              'The referenced field "addressId" does not have a unique index which is required to be used in a one-to-one relation.',
            ),
          );
        },
      );
  ...
  group(
    ...
    () {
      ...
      test(
        'then an error is collected that only one side may store the foreign key.',
        () {
          expect(
            collector.errors.map((error) => error.message),
            contains(
              'Only one side of the relation is allowed to store the foreign key, remove the "fk" property from one side.',
            ),
          );
        },
      );
