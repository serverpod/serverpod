...
void main() {
  group('Given generated code', () {
    ...
    test(
      'then all code contains ignore linting for public_member_api_docs',
      () {
        for (var code in codeMap.values) {
          expect(code, contains('// ignore_for_file: public_member_api_docs'));
        }
      },
    );

    test(
      'then all code contains ignore linting for implementation_imports',
      () {
        for (var code in codeMap.values) {
          expect(code, contains('// ignore_for_file: implementation_imports'));
        }
      },
    );
  });
