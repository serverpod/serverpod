...
void main() {
  ...
  group(
    ...
    test('when negate matching wrong initializer then test passes', () {
      expect(
        ...
        isNot(containsClass('User')
            .withUnnamedConstructor()
            .withFieldInitializer('invalidInitializer')),
      );
    });

    test('when matching field initializer then mismatch description is correct',
        () {
      final matcher = containsClass('User')
          .withUnnamedConstructor()
          .withFieldInitializer('wrongName') as Matcher;
  ...
  test(
    ...
    final matcher = containsClass('User')
        .withUnnamedConstructor()
        .withFieldInitializer('name') as Matcher;
    final description = StringDescription();
