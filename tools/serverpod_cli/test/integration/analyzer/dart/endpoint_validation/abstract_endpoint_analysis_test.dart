...
''');
      ...
      test(
        ...
        () {
          expect(endpointDefinitions.map((e) => e.className).toSet(), {
            'BaseEndpoint',
            'ConcreteEndpoint',
          });
        },
      ...
      test('then concrete endpoint has both base and concrete methods.', () {
        expect(concreteEndpoint.methods.map((m) => m.name).toSet(), {
          'baseMethod',
          'concreteMethod',
        });
      });
...
''');
      ...
      test('then only public endpoints are included in definitions.', () {
        expect(endpointDefinitions.map((e) => e.className).toSet(), {
          'PublicBaseEndpoint',
          'ConcreteImplementationEndpoint',
        });
      });
      ...
      test('then concrete implementation has all inherited methods.', () {
        expect(concrete.methods.map((m) => m.name).toSet(), {
          'hiddenMethod',
          'publicBaseMethod',
