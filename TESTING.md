...
One-time prerequisite: install [Melos](https://melos.invertase.dev) with
`dart pub global activate melos`, then resolve the workspace with `dart pub get`
from the repo root (re-run it after dependency changes). `melos run test` runs the primary host
...
`test_integration_modules`, and `test_integration_sqlite_server` (see the
`melos:` section in `pubspec.yaml`). The sections below are for running suites
directly with `dart test`.
